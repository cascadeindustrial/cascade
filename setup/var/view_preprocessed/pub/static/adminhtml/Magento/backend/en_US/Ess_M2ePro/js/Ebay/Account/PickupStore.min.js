define(['jquery','mage/apply/main','M2ePro/Common'],function(jQuery,mage){window.EbayPickupStore=Class.create(Common,{autogenerate:false,geocodeObject:{},defaultBusinessHours:{},initialize:function(){this.setValidationCheckRepetitionValue('M2ePro-pickup-store-location-id',M2ePro.translator.translate('The specified Title is already used for another In-Store Pickup. In-Store Pickup Title must be unique.'),'Ebay\\Account\\PickupStore','location_id','id',M2ePro.formData.id);jQuery.validator.addMethod('M2ePro-pickup-store-location-id-length',function(value){return value.length<=32;},M2ePro.translator.translate('Max length 32 character.'));jQuery.validator.addMethod('M2ePro-validate-must-arrive-date',function(value,el){if(el.up('#special_hours_grid')&&!el.up('#special_hours_grid').visible()){return true;}
...
var now=new Date(),fromTime=new Date(now.toDateString()+' '+$(el.id.replace('validator_to','from')).value),toTime=new Date(now.toDateString()+' '+$(el.id.replace('validator_to','to')).value);return(toTime-fromTime)>0;},M2ePro.translator.translate('Must be greater than "Open Time".'));jQuery.validator.addMethod('M2ePro-validate-max-length-128',function(value){return value.length<=128;},M2ePro.translator.translate('Max length 128 character.'));this.validateLocation();},init:function(){this.observeFields();},observeFields:function(){var self=this;if($('google_map').href.indexOf('#empty')!=-1){$('google_map').hide();}
...
topElement.up('tr').down('#'+inputId).value=hours+':'+minutes+':00';},addDateColumn:function(event){event.preventDefault();var self=EbayPickupStoreObj,template=$('special_hours_template'),specialHoursFieldsCount=++($$('.date-field').length);if(!$('special_hours_grid').visible()){self.showSpecialHoursGrid();}
...
+specialHoursFieldsCount;$('special_hours_grid_rows').down('tbody').appendChild(newColumn);newColumn.down('.remove_date').observe('click',self.removeDateColumn);newColumn.getElementsBySelector('.date_time_select').each(function(el){el.observe('change',self.scheduleTimeSelectChange).simulate('change');});var parent=newColumn.down('#date').up();parent.innerHTML='';var newElementKey='date-'+specialHoursFieldsCount;parent.insert({top:new Element('input',{type:'text',id:newElementKey,value:'',name:newElementKey,class:'input-text date-field M2ePro-required-when-visible M2ePro-validate-must-arrive-date','data-mage-init':'{"calendar": '+'{"showTime": false, "dateFormat": "yy-mm-dd", "showButtonPanel": false}'+'}',onchange:'EbayPickupStoreObj.dateOnChange(this)'})});mage.apply();},dateOnChange:function(el){var dateContainer=el,dateStr=dateContainer.value,dateFrom=dateContainer.up('tr').down('#date_from'),dateTo=dateContainer.up('tr').down('#date_to');dateFrom.name='special_hours[date_settings]['+dateStr+'][open]';dateTo.name='special_hours[date_settings]['+dateStr+'][close]';},removeDateColumn:function(event){var self=EbayPickupStoreObj;event.preventDefault();event.target.up('tr').remove();self.hideSpecialHoursGrid();},changeDefaultMode:function(){var self=EbayPickupStoreObj;if(+this.value){$('magento_block_ebay_account_pickup_store_form_data_quantity_custom_settings').show();var mode=M2ePro.formData.qty_mode;if(mode==M2ePro.php.constant('Ess_M2ePro_Model_Ebay_Account_PickupStore::QTY_MODE_SELLING_FORMAT_TEMPLATE')){M2ePro.formData.qty_mode=M2ePro.php.constant('Ess_M2ePro_Model_Ebay_Account_PickupStore::QTY_MODE_PRODUCT');}}else{$('magento_block_ebay_account_pickup_store_form_data_quantity_custom_settings').hide();}},qtyModeChange:function(){var self=EbayPickupStoreObj,customValueTr=$('qty_mode_cv_tr'),attributeElement=$('qty_custom_attribute'),maxPostedValueTr=$('qty_modification_mode_tr'),maxPostedValueMode=$('qty_modification_mode');customValueTr.hide();attributeElement.value='';if(isNaN(+this.value)||$('default_mode').value=='0'){return;}
...
maxPostedValueMode.simulate('change');self.updateQtyPercentage();},isMaxPostedQtyAvailable:function(qtyMode){return qtyMode==M2ePro.php.constant('Ess_M2ePro_Model_Ebay_Account_PickupStore::QTY_MODE_PRODUCT')||qtyMode==M2ePro.php.constant('Ess_M2ePro_Model_Ebay_Account_PickupStore::QTY_MODE_ATTRIBUTE')||qtyMode==M2ePro.php.constant('Ess_M2ePro_Model_Ebay_Account_PickupStore::QTY_MODE_PRODUCT_FIXED');},qtyPostedModeChange:function(){var minPosterValueTr=$('qty_min_posted_value_tr'),maxPosterValueTr=$('qty_max_posted_value_tr');minPosterValueTr.hide();maxPosterValueTr.hide();if(this.value==1){minPosterValueTr.show();maxPosterValueTr.show();}},updateQtyPercentage:function(){var qtyPercentageTr=$('qty_percentage_tr');qtyPercentageTr.hide();var qtyMode=$('qty_mode').value;if(qtyMode==M2ePro.php.constant('Ess_M2ePro_Model_Ebay_Account_PickupStore::QTY_MODE_SINGLE')||qtyMode==M2ePro.php.constant('Ess_M2ePro_Model_Ebay_Account_PickupStore::QTY_MODE_NUMBER')){return;}
qtyPercentageTr.show();},hideSpecialHoursGrid:function(){if($$('.date-field').length>0){return;}
$('special_hours_grid').hide();var addDateBtn=$$('.add_date').shift();addDateBtn.removeClassName('right');addDateBtn.style.width='';addDateBtn.up('div').removeClassName('add_btn_wrapper').addClassName('add_btn_wrapper_center');},showSpecialHoursGrid:function(){$('special_hours_grid').show();var addDateBtn=$$('.add_date').shift();addDateBtn.addClassName('right');addDateBtn.style.width='12.5%';addDateBtn.up('div').removeClassName('add_btn_wrapper_center').addClassName('add_btn_wrapper');},duplicateClick:function($headId,chapter_when_duplicate_text){$$('.loading-mask').invoke('show');this.setValidationCheckRepetitionValue('M2ePro-pickup-store-location-id',M2ePro.translator.translate('The specified Title is already used for another In-Store Pickup. In-Store Pickup Title must be unique.'),'Ebay\\Account\\PickupStore','location_id','id',0);M2ePro.formData.id=0;$('name').value=$('location_id').value=$('pickup_store_id').value='';$('location_id').disabled=false;if(!$('auto_generate_field').visible()){$('auto_generate_field').show();$('auto_generate').observe('change',EbayPickupStoreObj.switchAutoGeneration).simulate('change');}
$$('.page-title').each(function(o){o.innerHTML=chapter_when_duplicate_text;});$$('.M2ePro_duplicate_button').each(function(o){o.hide();});$$('.M2ePro_delete_button').each(function(o){o.hide();});window.setTimeout(function(){$$('.loading-mask').invoke('hide')},1200);},isValidForm:function(){var result=jQuery('#edit_form').valid();if(result){$('location_id').disabled=false;}
return result;},validateLocation:function(){jQuery.validator.addMethod('M2ePro-check-location',function(value){var preValidation=true;['country','region','city','address_1','postal_code','latitude','longitude'].each(function(el){if(!$(el)||$(el).value==''){preValidation=false;}});if(!preValidation){return true;}
