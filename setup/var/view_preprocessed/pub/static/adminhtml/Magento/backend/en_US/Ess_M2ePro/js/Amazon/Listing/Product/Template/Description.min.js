define(['Magento_Ui/js/modal/modal','M2ePro/Plugin/Messages','M2ePro/Action'],function(modal,MessageObj){window.AmazonListingProductTemplateDescription=Class.create(Action,{initialize:function($super,gridHandler){var self=this;$super(gridHandler);},options:{},setOptions:function(options){this.options=Object.extend(this.options,options);return this;},mapToTemplateDescription:function(el,templateId,mapToGeneralId){var self=this;self.confirm({actions:{confirm:function(){new Ajax.Request(M2ePro.url.get('amazon_listing_product_template_description/assign'),{method:'post',parameters:{products_ids:self.templateDescriptionPopup.productsIds,template_id:templateId},onSuccess:function(transport){if(!transport.responseText.isJSON()){self.alert(transport.responseText);return;}
...
clearInterval(intervalId);amazonTemplateDescriptionGridJsObject.reload();},1000);}});});