define(['jquery','Magento_Ui/js/modal/modal','M2ePro/Plugin/Messages','M2ePro/Common'],function(jQuery,modal,MessagesObj){window.ListingOtherMapping=Class.create(Common,{initialize:function(gridHandler,component){this.gridHandler=gridHandler;this.component=component;this.attachEvents();},openPopUp:function(productTitle,otherProductId){this.attachEvents();this.gridHandler.unselectAll();var modalDialogMessage=$('map_modal_dialog_message');if(!modalDialogMessage){modalDialogMessage=new Element('div',{id:'map_modal_dialog_message'});}
this.popUp=jQuery(modalDialogMessage).modal({title:M2ePro.translator.translate('Map Item "%product_title%" with Magento Product',productTitle),type:'slide',buttons:[]});this.popUp.modal('openModal');var content=$('pop_up_content');modalDialogMessage.insert(content.show());$('other_product_id').value=otherProductId;},attachEvents:function(){var self=this;$('mapping_submit_button').stopObserving('click').observe('click',function(event){self.map();});$('product_id').stopObserving('keypress').observe('keypress',function(event){event.keyCode==Event.KEY_RETURN&&self.map();});$('sku').stopObserving('keypress').observe('keypress',function(event){event.keyCode==Event.KEY_RETURN&&self.map();});},map:function(){var self=this;var productId=$('product_id').value;var sku=$('sku').value;var otherProductId=$('other_product_id').value;MessagesObj.clear();if(otherProductId==''||(/^\s*(\d)*\s*$/i).test(otherProductId)==false){return;}
