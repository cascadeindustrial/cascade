define(['Magento_Ui/js/modal/modal','M2ePro/Plugin/Messages','M2ePro/Action'],function(modal,MessageObj){window.AmazonListingProductTemplateShipping=Class.create(Action,{initialize:function($super,gridHandler){var self=this;$super(gridHandler);},options:{},setOptions:function(options){this.options=Object.extend(this.options,options);return this;},openPopUp:function(productsIds,shippingMode){var self=this;self.gridHandler.unselectAll();new Ajax.Request(M2ePro.url.get('amazon_listing_product_template_shipping/viewPopup'),{method:'post',parameters:{products_ids:productsIds,shipping_mode:shippingMode},onSuccess:function(transport){if(!transport.responseText.isJSON()){self.alert(transport.responseText);return;}
...
clearInterval(intervalId);self.loadGrid(shippingMode);},1000);}});});