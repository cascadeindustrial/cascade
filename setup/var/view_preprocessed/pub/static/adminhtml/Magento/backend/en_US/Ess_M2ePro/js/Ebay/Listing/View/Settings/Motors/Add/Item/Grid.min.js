...
$(self.gridId).down('.data-grid-filters').down('[name=year[from]]').value=item.value.from;$(self.gridId).down('.data-grid-filters').down('[name=year[to]]').value=item.value.to;});});self.getGridObj().doFilter();},massactionOnChange:function(event){if(event.target.value!='selectAll'){return true;}
