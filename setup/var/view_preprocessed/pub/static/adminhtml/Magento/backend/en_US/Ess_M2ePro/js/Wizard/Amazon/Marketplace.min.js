define(['M2ePro/Plugin/Messages','M2ePro/Marketplace'],function(MessageObj){window.WizardAmazonMarketplace=Class.create(Marketplace,{proceedAction:function(){MessageObj.clear();if(!this.checkAllOptions()){return;}
...
self.synchProgressObj.runTask(titleProgressBar,M2ePro.url.get('runSynchNow',{'marketplace_id':marketplaceId}),'','WizardAmazonMarketplaceObj.runNextMarketplaceNow();');return true;},checkAllOptions:function(){var isAnyOptionsEnabled=$$('select[id*="status_"]').some(function(element){return element.value==1;});if(!isAnyOptionsEnabled){MessageObj.addErrorMessage(M2ePro.translator.translate('You must select at least one Marketplace you will work with.'));CommonObj.scrollPageToTop();return false;}
return true;},disableAllOptions:function(){var allSelect=$$('select[id*="status_"]');if(allSelect.length){allSelect.each(function(element){element.disabled=true;});}}});});