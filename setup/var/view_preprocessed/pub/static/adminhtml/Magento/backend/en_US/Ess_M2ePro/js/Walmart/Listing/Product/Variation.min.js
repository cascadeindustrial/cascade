define(['Magento_Ui/js/modal/modal','M2ePro/Plugin/Messages','M2ePro/Common'],function(modal,MessageObj){window.WalmartListingProductVariation=Class.create();WalmartListingProductVariation.prototype=Object.extend(new Common(),{initialize:function(gridHandler){this.gridHandler=gridHandler;},setListingProductId:function(listingProductId){this.listingProductId=listingProductId;return this;},setNeededVariationData:function(variationAttributes,variationsTree){this.variationAttributes=variationAttributes;this.variationsTree=variationsTree;return this;},showSwitchToIndividualModePopUp:function(title){var self=this;if(self.switchToIndividualModePopUp){self.switchToIndividualModePopUp.modal('openModal');return;}
new Ajax.Request(M2ePro.url.get('walmart_listing_product_variation/getSwitchToIndividualModePopUp'),{method:'post',onSuccess:function(transport){$('html-body').insert({bottom:transport.responseText});self.switchToIndividualModePopUp=jQuery('#switch_to_individual_popup');modal({title:M2ePro.translator.translate('switch_to_individual_mode_popup_title'),type:'popup',modalClass:'width-50',buttons:[{text:M2ePro.translator.translate('No'),class:'action-secondary action-dismiss',click:function(){self.switchToIndividualModePopUp.modal('closeModal');}},{text:M2ePro.translator.translate('Yes'),class:'action-primary action-accept',click:function(){if($('switch_to_individual_remember_checkbox').checked){new Ajax.Request(M2ePro.url.get('saveListingAdditionalData'),{method:'post',parameters:{param_name:'hide_switch_to_individual_confirm',param_value:1},onSuccess:function(transport){self.gridHandler.unselectAllAndReload();}});}
...
return values;}
for(var filterAttribute in filters){if(filterAttribute==treeAttribute){return this.getAttributeValues(attribute,attributesTree[treeAttribute][filters[filterAttribute]],filters)}}}},editAction:function(variationData){MessageObj.clear();var parameters=Object.extend({listing_product_id:this.listingProductId},variationData);new Ajax.Request(M2ePro.url.get('walmart_listing_product_variation_individual/edit'),{method:'post',parameters:parameters,onSuccess:(function(transport){try{this.editPopup.modal('closeModal');var response=transport.responseText.evalJSON();MessageObj['add'+response.type[0].toUpperCase()+response.type.slice(1)+'Message'](response.message);if(response.type=='error'){this.scrollPageToTop();}else{this.gridHandler.unselectAllAndReload();}}catch(e){console.log(e.stack);this.scrollPageToTop();this.editPopup.modal('closeModal');MessageObj.addErrorMessage('Internal Error.');}}).bind(this)});},manageAction:function(variationData){MessageObj.clear();var parameters=Object.extend({listing_product_id:this.listingProductId},variationData);new Ajax.Request(M2ePro.url.get('walmart_listing_product_variation_individual/manage'),{method:'post',parameters:parameters,onSuccess:(function(transport){try{this.managePopup.modal('closeModal');var response=transport.responseText.evalJSON();MessageObj['add'+response.type[0].toUpperCase()+response.type.slice(1)+'Message'](response.message);if(response.type=='error'){this.scrollPageToTop();}else{this.gridHandler.unselectAllAndReload();}}catch(e){this.scrollPageToTop();this.managePopup.modal('closeModal');MessageObj.addErrorMessage('Internal Error.');}}).bind(this)});},manageGenerateAction:function(unique){var self=this,attributesIndexes={};$('variation_manage').select('th.data-grid-th').each(function(el,i){attributesIndexes[el.readAttribute('attribute').toLowerCase()]=i;});new Ajax.Request(M2ePro.url.get('walmart_listing_product_variation_individual/generate'),{method:'post',parameters:{listing_product_id:self.listingProductId,unique:+(unique)},onSuccess:function(transport){try{var response=transport.responseText.evalJSON();if(response.type=='error'){MessageObj.addErrorMessage(response.message);self.managePopup.modal('closeModal');return self.scrollPageToTop();}
