define(['Magento_Ui/js/modal/modal','M2ePro/Plugin/Messages','M2ePro/Action'],function(modal,MessageObj){window.AmazonListingProductSearch=Class.create(Action,{MATCHING_TYPE_EQUAL:1,MATCHING_TYPE_VIRTUAL_AMAZON:2,MATCHING_TYPE_VIRTUAL_MAGENTO:3,searchData:{},initialize:function($super,gridHandler){var self=this;$super(gridHandler);this.initValidators();self.searchBlock=$('productSearch_pop_up_content').outerHTML;$('productSearch_pop_up_content').remove();},initSearchEvents:function(){var self=this;$('productSearch_submit_button').observe('click',function(event){self.searchGeneralIdManual(self.params.productId);});$('productSearch_reset_button').observe('click',function(event){$('query').value='';$('productSearch_grid').innerHTML='';});$('query').observe('keypress',function(event){event.keyCode==Event.KEY_RETURN&&self.searchGeneralIdManual(self.params.productId);});},initValidators:function(){Validation.add('M2ePro-amazon-attribute-unique-value',M2ePro.translator.translate('variation_manage_matched_attributes_error_duplicate'),function(value,el){var existedValues=[],isValid=true,form=el.up('form');form.select('select').each(function(el){if(el.value!=''){if(existedValues.indexOf(el.value)===-1){existedValues.push(el.value);}else{isValid=false;}}});return isValid;});},params:{autoMapErrorFlag:false},openPopUp:function(mode,title,productId){MessageObj.clear();var self=this;this.gridHandler.unselectAll();this.params={mode:mode,title:title,productId:productId,autoMapErrorFlag:false};if(mode==0){new Ajax.Request(M2ePro.url.get('amazon_listing_product_search/getSearchAsinMenu'),{method:'post',parameters:{product_id:productId},onSuccess:function(transport){var containerEl=$('productSearchMenu_pop_up_content');if(containerEl){containerEl.remove();}
