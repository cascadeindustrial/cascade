...
this._createPopup(popupMode,title,content,popupData);},_createPopup:function(mode,title,content,popupData){var self=this,modalDialogMessage=$(mode+'modal_auto_action'),buttonsConfig=[];if(!modalDialogMessage){modalDialogMessage=new Element('div',{id:mode+'modal_auto_action'});}
...
template.innerHTML=$('dialog_confirm_content').innerHTML.replace('%s',this.magentoCategoryIdsFromOtherGroups[latestCategory].title);jQuery(template).confirm({title:M2ePro.translator.translate('Remove Category'),actions:{confirm:function(){new Ajax.Request(M2ePro.url.get(ListingAutoActionObj.controller+'/deleteCategory'),{method:'post',asynchronous:true,parameters:{group_id:ListingAutoActionObj.magentoCategoryIdsFromOtherGroups[latestCategory].id,category_id:latestCategory},onSuccess:function(transport){delete ListingAutoActionObj.magentoCategoryIdsFromOtherGroups[latestCategory];}});return true;},cancel:function(){tree.getNodeById(latestCategory).ui.check(false);}},buttons:[{text:M2ePro.translator.translate('Cancel'),class:'action-secondary action-dismiss',click:function(event){this.closeModal(event);}},{text:M2ePro.translator.translate('Confirm'),class:'action-primary action-accept',click:function(event){this.closeModal(event,true);}}],closed:function(){ListingAutoActionObj.magentoCategoryTreeChangeEventInProgress=false;}});},highlightBreadcrumbStep:function(step){$$('[id$="breadcrumb_container"] .breadcrumb').each(function(element){element.removeClassName('selected');});$('step_'+step).addClassName('selected');},globalStepTwo:function(){ListingAutoActionObj.collectData();var callback=function(){jQuery('#continue_button').off('click').on('click',ListingAutoActionObj.globalStepThree);ListingAutoActionObj.highlightBreadcrumbStep(2);};ListingAutoActionObj.loadCategoryChooser(callback);},globalStepThree:function(){if(!ListingCategoryChooserHandlerObj.validate()){return;}
ListingAutoActionObj.collectData();var callback=function(){ListingAutoActionObj.highlightBreadcrumbStep(3);$('confirm_button').show();$('continue_button').hide();};ListingAutoActionObj.loadSpecific(callback);},websiteStepTwo:function(){ListingAutoActionObj.collectData();var callback=function(){jQuery('#'+ListingAutoActionObj.getPopupMode()+'continue_button').off('click').on('click',ListingAutoActionObj.websiteStepThree);ListingAutoActionObj.highlightBreadcrumbStep(2);};ListingAutoActionObj.loadCategoryChooser(callback);},websiteStepThree:function(){if(!ListingCategoryChooserHandlerObj.validate()){return;}
ListingAutoActionObj.collectData();var callback=function(){ListingAutoActionObj.highlightBreadcrumbStep(3);$(ListingAutoActionObj.getPopupMode()+'confirm_button').show();$(ListingAutoActionObj.getPopupMode()+'continue_button').hide();};ListingAutoActionObj.loadSpecific(callback);},isCategoryAlreadyUsed:function(categoryId){return this.magentoCategoryUsedIds.indexOf(categoryId)!=-1;},categoryCancel:function(){ListingAutoActionObj.loadAutoActionHtml(M2ePro.php.constant('Ess_M2ePro_Model_Listing::AUTO_MODE_CATEGORY'));},categoryStepOne:function(groupId){var mode=ListingAutoActionObj.getPopupMode();this.loadAutoCategoryForm(groupId,function(){$(mode+'reset_button').hide();$(mode+'close_button').hide();$(mode+'cancel_button').show();});},categoryStepTwo:function(){if(!ListingAutoActionObj.validate()){return;}
ListingAutoActionObj.collectData();var mode=ListingAutoActionObj.getPopupMode();var callback=function(){jQuery('#'+mode+'continue_button').off('click').on('click',ListingAutoActionObj.categoryStepThree);ListingAutoActionObj.highlightBreadcrumbStep(2);};ListingAutoActionObj.loadCategoryChooser(callback);},categoryStepThree:function(){ListingAutoActionObj.collectData();var mode=ListingAutoActionObj.getPopupMode();var callback=function(){ListingAutoActionObj.highlightBreadcrumbStep(3);$(mode+'confirm_button').show();$(mode+'continue_button').hide();};},categoryDeleteGroup:function(groupId){this.confirmPopup(M2ePro.translator.translate('Are you sure?'),{confirmCallback:function(){new Ajax.Request(M2ePro.url.get(ListingAutoActionObj.controller+'/deleteCategoryGroup'),{method:'post',asynchronous:true,parameters:{group_id:groupId},onSuccess:function(transport){listingAutoActionModeCategoryGroupGridJsObject.doFilter();}});}});},validate:function(){var validationResult=true;this.currentPopup.find('form').each(function(){validationResult=validationResult&&jQuery(this).validation().valid();});return validationResult;},confirm:function(){if(!ListingAutoActionObj.validate()){return;}
...
ListingAutoActionObj.submitData(callback);},collectData:function(){if($('auto_mode')){switch(parseInt($('auto_mode').value)){case M2ePro.php.constant('Ess_M2ePro_Model_Listing::AUTO_MODE_GLOBAL'):ListingAutoActionObj.internalData={auto_mode:$('auto_mode').value,auto_global_adding_mode:$('auto_global_adding_mode').value,auto_global_adding_add_not_visible:$('auto_global_adding_add_not_visible').value,};break;case M2ePro.php.constant('Ess_M2ePro_Model_Listing::AUTO_MODE_WEBSITE'):ListingAutoActionObj.internalData={auto_mode:$('auto_mode').value,auto_website_adding_mode:$('auto_website_adding_mode').value,auto_website_adding_add_not_visible:$('auto_website_adding_add_not_visible').value,auto_website_deleting_mode:$('auto_website_deleting_mode').value};break;case M2ePro.php.constant('Ess_M2ePro_Model_Listing::AUTO_MODE_CATEGORY'):ListingAutoActionObj.internalData={id:$('group_id').value,title:$('group_title').value,auto_mode:$('auto_mode').value,adding_mode:$('adding_mode').value,adding_add_not_visible:$('adding_add_not_visible').value,deleting_mode:$('deleting_mode').value,categories:categories_selected_items};break;}}},submitData:function(callback){var data=this.internalData;new Ajax.Request(M2ePro.url.get(ListingAutoActionObj.controller+'/save'),{method:'post',asynchronous:true,parameters:{auto_action_data:Object.toJSON(data)},onSuccess:function(transport){if(typeof callback=='function'){callback();}}});},reset:function(skipConfirmation,callback,closeCallback){skipConfirmation=skipConfirmation||false;var confirmCallback=function(){new Ajax.Request(M2ePro.url.get(ListingAutoActionObj.controller+'/reset'),{method:'post',asynchronous:true,parameters:{},onSuccess:function(transport){callback&&callback();ListingAutoActionObj.loadAutoActionHtml();}});};if(skipConfirmation){confirmCallback();return}
ListingAutoActionObj.confirmPopup(M2ePro.translator.translate('Are you sure?'),{confirmCallback:confirmCallback,closeCallback:closeCallback});},confirmPopup:function(message,configData){jQuery('<div>').confirm({title:message,actions:{always:function(){configData.closeCallback&&configData.closeCallback();},confirm:function(){configData.confirmCallback&&configData.confirmCallback();return true;},buttons:[{text:M2ePro.translator.translate('Cancel')},{text:M2ePro.translator.translate('Confirm'),class:'primary'}]}});}});});