...
var response=transport.responseText.evalJSON();var data={};skus.each(function(item){if(response.items[item.sku]){data[item.productId]=response.items[item.sku].qty;}else{data[item.productId]='empty';}});self.renderAfnQty(data);}});},renderAfnQty:function(data){var self=this;$H(data).each(function(item){var container=$('m2ePro_afn_qty_value_'+item.key);container.down('a').hide();container.down('.m2ePro-online-sku-value').remove();if(item.value!='empty'){container.down('.m2epro-afn-qty-data').show();container.down('.total span').update(item.value.total);container.down('.in-stock span').update(item.value.in_stock);}else{container.down('.m2epro-empty-afn-qty-data').show();}});}};});