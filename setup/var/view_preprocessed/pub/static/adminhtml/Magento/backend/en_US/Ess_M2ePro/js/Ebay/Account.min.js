...
var isAttributeSelected=false;$$('.attribute-mode-select').each(function(obj){if(obj.value!=0){isAttributeSelected=true;}});return isAttributeSelected;},M2ePro.translator.translate('If Yes is chosen, you must select at least one Attribute for Product Mapping.'));},initObservers:function(){if(!M2ePro.formData.id){return;}
$('other_listings_synchronization').observe('change',this.other_listings_synchronization_change).simulate('change');$('other_listings_mapping_mode').observe('change',this.other_listings_mapping_mode_change).simulate('change');$('mapping_sku_mode').observe('change',this.mapping_sku_mode_change).simulate('change');$('mapping_title_mode').observe('change',this.mapping_title_mode_change).simulate('change');$('mapping_item_id_mode').observe('change',this.mapping_item_id_mode_change).simulate('change');$('magento_orders_listings_mode').observe('change',this.magentoOrdersListingsModeChange).simulate('change');$('magento_orders_listings_store_mode').observe('change',this.magentoOrdersListingsStoreModeChange).simulate('change');$('magento_orders_listings_other_mode').observe('change',this.magentoOrdersListingsOtherModeChange).simulate('change');$('magento_orders_listings_other_product_mode').observe('change',this.magentoOrdersListingsOtherProductModeChange);$('magento_orders_number_source').observe('change',this.magentoOrdersNumberSourceChange);$('magento_orders_number_prefix_mode').observe('change',this.magentoOrdersNumberPrefixModeChange).simulate('change');$('magento_orders_number_prefix_prefix').observe('keyup',this.magentoOrdersNumberPrefixPrefixChange).simulate('change');EbayAccountObj.renderOrderNumberExample();$('magento_orders_customer_mode').observe('change',this.magentoOrdersCustomerModeChange).simulate('change');$('magento_orders_status_mapping_mode').observe('change',this.magentoOrdersStatusMappingModeChange);$('magento_orders_creation_mode').observe('change',this.magentoOrdersCreationModeChange).simulate('change');$('order_number_example-note').previous().remove();},saveAndClose:function(){var self=this,url=typeof M2ePro.url.urls.formSubmit=='undefined'?M2ePro.url.formSubmit+'back/'+Base64.encode('list')+'/':M2ePro.url.get('formSubmit',{'back':Base64.encode('list')});if(!this.isValidForm()){return;}
new Ajax.Request(url,{method:'post',parameters:Form.serialize($('edit_form')),onSuccess:function(transport){transport=transport.responseText.evalJSON();if(transport.success){window.close();}else{self.alert(transport.message);}}});},deleteClick:function(){this.confirm({content:M2ePro.translator.translate('Be attentive! By Deleting Account you delete all information on it from M2E Pro Server. This will cause inappropriate work of all Accounts\' copies.'),actions:{confirm:function(){setLocation(M2ePro.url.get('deleteAction'));},cancel:function(){return false;}}});},get_token:function(){if($('token_session').value==''){$('token_session').value='0';}
if($('token_expired_date').value==''){$('token_expired_date').value='0';}
if(!jQuery('#edit_form').valid()){return;}
this.submitForm(M2ePro.url.get('ebay_account/beforeGetToken',{'id':M2ePro.formData.id}));},feedbacksReceiveChange:function(){var self=EbayAccountObj;if($('feedbacks_receive').value==M2ePro.php.constant('\\Ess\\M2ePro\\Model\\Ebay\\Account::FEEDBACKS_RECEIVE_YES')){$('feedbacks_auto_response_container').show();}else{$('feedbacks_auto_response_container').hide();}
$('feedbacks_auto_response').value=M2ePro.php.constant('\\Ess\\M2ePro\\Model\\Ebay\\Account::FEEDBACKS_AUTO_RESPONSE_NONE');self.feedbacksAutoResponseChange();},feedbacksAutoResponseChange:function(){if($('feedbacks_auto_response').value==M2ePro.php.constant('\\Ess\\M2ePro\\Model\\Ebay\\Account::FEEDBACKS_AUTO_RESPONSE_NONE')){$('feedbacks_auto_response_only_positive_container').hide();$('feedback_templates_grid_container').hide();}else{$('feedbacks_auto_response_only_positive_container').show();$('feedback_templates_grid_container').show();}},openFeedbackTemplatePopup:function(templateId){var self=this;new Ajax.Request(M2ePro.url.get('ebay_account_feedback_template/getForm'),{method:'GET',parameters:{id:templateId},onSuccess:function(transport){var response=transport.responseText.evalJSON();var container=$('edit_feedback_template_form_container');if(container){container.remove();}
