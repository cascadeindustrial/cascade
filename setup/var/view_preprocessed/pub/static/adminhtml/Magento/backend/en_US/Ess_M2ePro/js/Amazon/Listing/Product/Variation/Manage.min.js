define(['M2ePro/Plugin/Messages','Magento_Ui/js/modal/modal','M2ePro/Action'],function(MessageObj,modal){window.AmazonListingProductVariationManage=Class.create(Action,{MATCHING_TYPE_EQUAL:1,MATCHING_TYPE_VIRTUAL_AMAZON:2,MATCHING_TYPE_VIRTUAL_MAGENTO:3,initialize:function($super,gridHandler){$super(gridHandler);this.initValidators();},matchingType:1,matchedAttributes:[],productAttributes:[],destinationAttributes:[],selectedDestinationAttributes:[],magentoVariationSet:[],amazonVariationSet:false,initValidators:function(){Validation.add('M2ePro-amazon-attribute-unique-value',M2ePro.translator.translate('variation_manage_matched_attributes_error_duplicate'),function(value,el){var existedValues=[],isValid=true,form=el.up('form');form.select('select').each(function(el){if(el.value!=''){if(existedValues.indexOf(el.value)===-1){existedValues.push(el.value);}else{isValid=false;}}});return isValid;});},initSettingsTab:function(){var self=this,form=$('variation_manager_attributes_form');if(form&&self.matchingType==self.MATCHING_TYPE_EQUAL){form.observe('change',function(e){if(e.target.tagName!='SELECT'){return;}
