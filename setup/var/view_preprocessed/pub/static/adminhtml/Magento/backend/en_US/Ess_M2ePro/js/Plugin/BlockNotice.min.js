define(['jquery','M2ePro/Plugin/Storage','M2ePro/Common'],function(jQuery,localStorage){window.BlockNotice=Class.create(Common,{initializedBlocks:[],storageKeys:{prefix:'m2e_bn_',shown:'_was_shown',closed:'_closed',hiddenContent:'_hidden_content',expandedContent:'_expanded_content',},getHashedStorage:function(id){var hashedStorageKey=this.storageKeys.prefix+md5(id).substr(0,10);var resultStorage=localStorage.get(hashedStorageKey);if(resultStorage===null){return'';}
return resultStorage;},setHashedStorage:function(id){var hashedStorageKey=this.storageKeys.prefix+md5(id).substr(0,10);localStorage.set(hashedStorageKey,1);},deleteHashedStorage:function(id){var hashedStorageKey=this.storageKeys.prefix+md5(id).substr(0,10);localStorage.remove(hashedStorageKey);localStorage.remove(id);},deleteAllHashedStorage:function(){localStorage.removeAllByPrefix(this.storageKeys.prefix);},showContent:function(id,storage){id=id||'';if(id==''){return false;}
...
var block=jQuery('#'+id);block.hide(0,function(){self.updateFloatingHeader();block.remove();});self.setHashedStorage(id+self.storageKeys.closed);},cancel:function(){return false;}}});},hideFieldsetIcon:function(id){var fieldsetId=id.split('help_block_')[1];jQuery('#'+fieldsetId+'-wrapper').find('.m2epro-fieldset-tooltip').hide();},init:function(reinit){if(typeof reinit==='undefined'){reinit=false;}
var blockNotices=jQuery('.block_notices');if(!blockNotices.length){return;}
var self=this;blockNotices.each(function(index,block){if(self.shouldBeHidden(block)){block.hide();return;}
var id=block.id;if(self.initializedBlocks.indexOf(id)>-1){return;}!reinit&&self.initializedBlocks.push(id);var noCollapse=jQuery(block).hasClass('no_collapse');if(noCollapse){self.showContent(id,false);return;}
var wasShown=self.getHashedStorage(id+self.storageKeys.shown);var tooltiped=jQuery(block).hasClass('tooltiped');var expanded=self.getHashedStorage(id+self.storageKeys.expandedContent);var closed=self.getHashedStorage(id+self.storageKeys.closed);if(!wasShown){self.showContent(id,false);self.setHashedStorage(id+self.storageKeys.shown);tooltiped&&self.hideFieldsetIcon(id);}else if(tooltiped){jQuery(block).hide();}else if(closed){jQuery(block).remove();}else if(expanded){self.showContent(id,false);}else{jQuery(block).show();}});},removeInitializedBlock:function(block){var index=this.initializedBlocks.indexOf(block);if(index>-1){this.initializedBlocks.splice(index,1);}},shouldBeHidden:function(block){if(typeof BLOCK_NOTICES_SHOW==='undefined'||BLOCK_NOTICES_SHOW){return false;}
