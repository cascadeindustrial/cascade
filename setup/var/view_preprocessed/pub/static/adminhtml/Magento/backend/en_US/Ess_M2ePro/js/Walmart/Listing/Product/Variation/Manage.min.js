define(['M2ePro/Plugin/Messages','Magento_Ui/js/modal/modal','M2ePro/Action'],function(MessageObj,modal){window.WalmartListingProductVariationManage=Class.create(Action,{MATCHING_TYPE_EQUAL:1,MATCHING_TYPE_VIRTUAL_WALMART:2,MATCHING_TYPE_VIRTUAL_MAGENTO:3,initialize:function($super,gridHandler){$super(gridHandler);this.initValidators();},matchingType:1,matchedAttributes:[],productAttributes:[],destinationAttributes:[],selectedDestinationAttributes:[],magentoVariationSet:[],walmartVariationSet:false,initValidators:function(){Validation.add('M2ePro-walmart-attribute-unique-value',M2ePro.translator.translate('variation_manage_matched_attributes_error_duplicate'),function(value,el){var existedValues=[],isValid=true,form=el.up('form');form.select('select').each(function(el){if(el.value!=''){if(existedValues.indexOf(el.value)===-1){existedValues.push(el.value);}else{isValid=false;}}});return isValid;});},initSettingsTab:function(){var self=this,form=$('variation_manager_attributes_form');if(form&&self.matchingType==self.MATCHING_TYPE_EQUAL){form.observe('change',function(e){if(e.target.tagName!='SELECT'){return;}
...
new Ajax.Request(M2ePro.url.get('walmart_listing_product_variation_vocabulary/addOptions'),{method:'post',parameters:{options_data:$('vocabulary_options_data').value,need_add:needAdd?1:0,is_remember:isRemember?1:0},onSuccess:function(transport){self.vocabularyOptionsPopup.modal('closeModal');}});},openSkuPopUp:function(){var self=this;new Ajax.Request(M2ePro.url.get('walmart_listing_product_variation_manage/getSkuPopup'),{method:'post',parameters:{product_id:self.variationProductManagePopup.productId},onSuccess:function(transport){var containerEl=$('manage_variation_sku_popup');if(containerEl){containerEl.remove();}
...
data=$('variation_manager_sku_form').serialize(true);data.product_id=self.variationProductManagePopup.productId;new Ajax.Request(M2ePro.url.get('walmart_listing_product_variation_manage/setListingProductSku'),{method:'post',parameters:data,onSuccess:function(transport){var response=self.parseResponse(transport);if(response.success){self.skuPopup.modal('closeModal');self.setGeneralIdOwner(1,true);}else{var skuError=$('sku-error');if(!skuError){skuError=new Element('label',{id:'sku-error',for:'sku',class:'error',generated:true});$('sku').insert({after:skuError})}
skuError.update(response.msg);skuError.show();}}});},changeChannelAttributes:function(el){$('channel_attributes_view').hide();$('variation_manager_channel_attributes_form').show();$('M2ePro-walmart-required-channel-attribute-error').hide();},setChannelAttributes:function(){var self=this,result=false;form=$('variation_manager_channel_attributes_form');$$('.M2ePro-walmart-required-channel-attribute').each(function(obj){if(obj.checked){result=true;return;}});if(result){var data=form.serialize(true);data.product_id=self.variationProductManagePopup.productId;new Ajax.Request(M2ePro.url.get('walmart_listing_product_variation_manage/setChannelAttributes'),{method:'post',parameters:data,onSuccess:function(transport){var response=self.parseResponse(transport);if(response.success){self.reloadVariationsGrid();}}});}else{var errorChannelAttribute=$('M2ePro-walmart-required-channel-attribute-error');errorChannelAttribute.show();errorChannelAttribute.update(M2ePro.translator.translate('At least one Variant Attribute must be selected.'))}},cancelChannelAttributes:function(el){$('channel_attributes_view').show();$('variation_manager_channel_attributes_form').hide();$('variation_manager_channel_attributes_form').reset();},setSwatchImagesAttribute:function(){var self=this,el=$('swatch_images_attributes');new Ajax.Request(M2ePro.url.get('walmart_listing_product_variation_manage/setSwatchImagesAttribute'),{method:'post',parameters:{product_id:self.variationProductManagePopup.productId,attribute:el.value},onSuccess:function(transport){var response=self.parseResponse(transport);if(response.success){self.reloadSettings();}}});},changeMatchedAttributes:function(el){$$('.variation_manager_attributes_walmart_value').each(function(el){el.hide();});$$('.variation_manager_attributes_walmart_select').each(function(el){el.show();});el.hide();el.next().show();el.next().next().show();},isValidAttributes:function(){var self=this,existedValues=[],isValid=true,form=$('variation_manager_attributes_form');if(!form||(form&&form.serialize()=='')){return true;}
var data=form.serialize(true);form.select('.validation-advice').each(function(el){el.hide();});if(typeof data['variation_attributes[walmart_attributes][]']=='string'){if(data['variation_attributes[walmart_attributes][]']!=''){return true;}
...
var i=0;data['variation_attributes[walmart_attributes][]'].each(function(attrVal){if(attrVal!=''&&existedValues.indexOf(attrVal)===-1){existedValues.push(attrVal);}else{isValid=false;var errorEl=$('variation_manager_attributes_error_'+i);errorEl.show();if(attrVal==''){errorEl.update(M2ePro.translator.translate('variation_manage_matched_attributes_error'));}else{errorEl.update(M2ePro.translator.translate('variation_manage_matched_attributes_error_duplicate'))}}
i++;});return isValid;},setMatchedAttributes:function(){var self=this,data;if(!self.isValidAttributes()){return;}
$('variation_manager_attributes_form').select('.validation-advice').each(function(el){el.hide();});data=$('variation_manager_attributes_form').serialize(true);data.product_id=self.variationProductManagePopup.productId;new Ajax.Request(M2ePro.url.get('walmart_listing_product_variation_manage/setMatchedAttributes'),{method:'post',parameters:data,onSuccess:function(transport){var response=self.parseResponse(transport);if(response.success){self.reloadVariationsGrid();if(response['vocabulary_attributes']){self.openVocabularyAttributesPopUp(response['vocabulary_attributes']);}}}});},cancelMatchedAttributes:function(el){$$('.variation_manager_attributes_walmart_value').each(function(el){el.show();});$$('.variation_manager_attributes_walmart_select').each(function(el){el.hide();});$('variation_manager_attributes_form').select('.validation-advice').each(function(el){el.hide();});el.hide();el.previous().show();el.next().hide();},renderMatchedAttributesNotSetView:function(type){var self=this,form=$('variation_manager_attributes_form'),tBody=form.down('tbody');tBody.update();$H(self.matchedAttributes).each(function(attribute){var tr=new Element('tr'),tdLabel=new Element('td',{class:'label',style:'border-right: 1px solid #D6D6D6 !important;'}),label=new Element('label'),tdValue=new Element('td',{class:'value'}),valueSpan=new Element('span',{style:'color: red;'});label.innerHTML=attribute.key;valueSpan.innerHTML=M2ePro.translator.translate('not_set');tdLabel.insert({bottom:label});tdValue.insert({bottom:valueSpan});tr.insert({bottom:tdLabel});tr.insert({bottom:tdValue});tBody.insert({bottom:tr});});var tr=new Element('tr'),tdBtns=new Element('td',{class:'label',colspan:'2',style:'text-align: right; border: none'}),setBtn=new Element('button',{class:'action primary'});setBtn.update(M2ePro.translator.translate('set_attributes'));setBtn.observe('click',function(event){if(self.matchingType===self.MATCHING_TYPE_VIRTUAL_WALMART){self.renderMatchedAttributesVirtualWalmartView();}
if(self.matchingType===self.MATCHING_TYPE_VIRTUAL_MAGENTO){self.renderMatchedAttributesVirtualMagentoView();}});tdBtns.insert({bottom:setBtn});tr.insert({bottom:tdBtns});tBody.insert({bottom:tr});},renderMatchedAttributesVirtualWalmartView:function(){var self=this,form=$('variation_manager_attributes_form'),tBody=form.down('tbody');tBody.update();self.selectedDestinationAttributes=[];var prematchedAttributes=[];var i=0;$H(self.matchedAttributes).each(function(attribute){var tr=new Element('tr'),tdLabel=new Element('td',{class:'label value',style:'border-right: 1px solid #D6D6D6 !important; vertical-align: inherit;'}),labelMagentoAttr=new Element('label',{class:'magento-attribute-name'}),inputVirtualAttribute=new Element('input',{style:'display: none',value:attribute.key,type:'hidden',disabled:'disabled',class:'virtual-walmart-attribute-name-value',name:'variation_attributes[virtual_walmart_attributes]['+i+']'}),selectVirtualAttributeOption=new Element('select',{style:'display: none; width: 200px; margin-top: -3px',disabled:'disabled',class:'admin__control-select required-entry virtual-walmart-option',name:'variation_attributes[virtual_walmart_option]['+i+']'}),selectVirtualAttributeOptionGroup=new Element('optgroup',{label:attribute.key}),labelVirtualAttributeAndOption=new Element('span',{style:'display: none; line-height: 20px; vertical-align: baseline;',class:'virtual-walmart-attribute-and-option'}),spanLeftHelpIcon=new Element('span',{style:'display: none',class:'left-help-icon'}),tdValue=new Element('td',{class:'value'}),inputMagentoAttr=new Element('input',{value:attribute.key,type:'hidden',class:'magento-attribute-name-value',name:'variation_attributes[magento_attributes]['+i+']'}),selectWalmartAttr=new Element('select',{class:'admin__control-select required-entry M2ePro-walmart-attribute-unique-value walmart-attribute-name',name:'variation_attributes[walmart_attributes]['+i+']'}),spanVirtualAttribute=new Element('span',{style:'display: none; line-height: 20px; vertical-align: baseline;',class:'virtual-walmart-attribute-name'}),spanRightHelpIcon=new Element('span',{style:'display: none',class:'right-help-icon'});var helpIconTpl=$('variation_settings_form_help_icon_tpl');spanLeftHelpIcon.update(helpIconTpl.innerHTML);spanLeftHelpIcon.down('.tool-tip-message-text').update(M2ePro.translator.translate('help_icon_magento_greater_left'));spanRightHelpIcon.update(helpIconTpl.innerHTML);spanRightHelpIcon.down('.tool-tip-message-text').update(M2ePro.translator.translate('help_icon_magento_greater_right'));var attributeStr=attribute.key;if(attribute.key.length>13){attributeStr=attribute.key.substr(0,12)+'...';labelVirtualAttributeAndOption.title=attribute.key;spanVirtualAttribute.title=attribute.key;}
...
tr.insert({bottom:tdLabel});tr.insert({bottom:tdValue});buttonsRow.insert({before:tr});i++;});form.select('.tool-tip-image').each(function(element){element.observe('mouseover',MagentoFieldTipObj.showToolTip);element.observe('mouseout',MagentoFieldTipObj.onToolTipIconMouseLeave);});form.select('.tool-tip-message').each(function(element){element.observe('mouseout',MagentoFieldTipObj.onToolTipMouseLeave);element.observe('mouseover',MagentoFieldTipObj.onToolTipMouseEnter);});},hideVirtualMagentoAttributes:function(){var self=this,form=$('variation_manager_attributes_form');form.select('tr.virtual-attribute').each(function(el){el.remove();});},reloadSettings:function(callback){var self=this;new Ajax.Request(M2ePro.url.get('walmart_listing_product_variation_manage/viewVariationsSettingsAjax'),{method:'post',parameters:{product_id:self.variationProductManagePopup.productId},onSuccess:function(transport){var response=self.parseResponse(transport);$('walmartVariationProductManageTabs_settings_content').update(response.html);self.initSettingsTab();if(callback){callback.call();}}});},reloadVariationsGrid:function(){ListingProductVariationManageVariationsGridObj.actionHandler.gridHandler.unselectAllAndReload();},openVariationsTab:function(createNewAsin){jQuery('#walmartVariationProductManageTabs').tabs('option','active',0);ListingProductVariationManageVariationsGridObj.showNewChildForm(createNewAsin,ListingGridHandlerObj.variationProductManageHandler.variationProductManagePopup.productId);},reloadVocabulary:function(){var self=this;new Ajax.Request(M2ePro.url.get('walmart_listing_product_variation_vocabulary/viewVocabularyAjax'),{method:'post',parameters:{product_id:self.variationProductManagePopup.productId},onSuccess:function(transport){$('walmartVariationProductManageTabs_vocabulary_content').update(transport.responseText);}});},saveAutoActionSettings:function(){new Ajax.Request(M2ePro.url.get('walmart_listing_product_variation_manage/saveAutoActionSettings'),{method:'post',parameters:$('auto_action_settings_form').serialize(true)});},removeAttributeFromVocabulary:function(el){var self=this;self.confirm({actions:{confirm:function(){new Ajax.Request(M2ePro.url.get('walmart_listing_product_variation_vocabulary/removeAttribute'),{method:'post',parameters:{magento_attr:decodeHtmlentities(el.up().down('.magento-attribute-name').innerHTML),channel_attr:decodeHtmlentities(el.up().down('.channel-attribute-name').innerHTML)},onSuccess:function(transport){self.reloadVocabulary();}});},cancel:function(){return false;}}});},removeOptionFromVocabulary:function(el){var self=this,optionGroupRowEl=el.up('.channel-attribute-options-group');self.confirm({actions:{confirm:function(){new Ajax.Request(M2ePro.url.get('walmart_listing_product_variation_vocabulary/removeOption'),{method:'post',parameters:{product_option:decodeHtmlentities(optionGroupRowEl.down('.product-option').innerHTML),product_options_group:decodeHtmlentities(optionGroupRowEl.down('.product-options-group').innerHTML),channel_attr:decodeHtmlentities(optionGroupRowEl.down('.channel-attribute-name').innerHTML)},onSuccess:function(transport){self.reloadVocabulary();}});},cancel:function(){return false;}}});}});});