define(['M2ePro/SynchProgress'],function(){MigrationFromMagento1MarketplaceSynchProgress=Class.create(SynchProgress,{start:function(title,status){title=title||'';status=status||'';var self=this;self.progressBarObj.reset();if(title!=''){self.progressBarObj.setTitle(title);}
...
self.progressBarObj.setStatus(data.status);setTimeout(function(){self.startGetExecutingInfo(callBackWhenEnd);},3000);}else{self.progressBarObj.setPercents(100,0);setTimeout(function(){if(callBackWhenEnd!=''){eval(callBackWhenEnd);}else{self.end();}},1500);}}});}});});