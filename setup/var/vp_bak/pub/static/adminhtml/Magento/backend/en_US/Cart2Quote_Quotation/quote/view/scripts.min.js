define(["jquery",'Magento_Ui/js/modal/confirm','Magento_Ui/js/modal/alert',"mage/translate","prototype","Magento_Catalog/catalog/product/composite/configure",'Magento_Ui/js/lib/view/utils/async'],function(jQuery,confirm,alert){window.AdminQuote=new Class.create();AdminQuote.prototype={initialize:function(data){if(!data){data={};}
this.loadBaseUrl=false;this.customerId=data.customer_id?data.customer_id:false;this.storeId=data.store_id?data.store_id:false;this.currencyId=false;this.currencySymbol=data.currency_symbol?data.currency_symbol:'';this.addresses=data.addresses?data.addresses:$H({});this.shippingAsBilling=data.shippingAsBilling?data.shippingAsBilling:false;this.gridProducts=$H({});this.gridProductsGift=$H({});this.billingAddressContainer='';this.shippingAddressContainer='';this.isShippingMethodReseted=data.shipping_method_reseted?data.shipping_method_reseted:false;this.overlayData=$H({});this.giftMessageDataChanged=false;this.productConfigureAddFields={};this.productPriceBase={};this.collectElementsValue=true;this.isOnlyVirtualProduct=false;this.excludedPaymentMethods=[];this.summarizePrice=true;jQuery.async('#quote-items',(function(){this.dataArea=new QuoteFormArea('data',$(this.getAreaId('data')),this);this.itemsArea=Object.extend(new QuoteFormArea('items',$(this.getAreaId('items')),this),{addControlButton:function(button){var controlButtonArea=$(this.node).select('.actions')[0];if(typeof controlButtonArea!='undefined'){var buttons=controlButtonArea.childElements();for(var i=0;i<buttons.length;i++){if(buttons[i].innerHTML.include(button.label)){return;}}
button.insertIn(controlButtonArea,'top');}}});var searchButtonId='add_products_to_quote';var searchButton=new ControlButton(jQuery.mage.__('Add Products'),searchButtonId);var searchAreaId=this.getAreaId('search');searchButton.onClick=function(){$(searchAreaId).show();var el=this;window.setTimeout(function(){el.remove();},10);};if(jQuery('#'+this.getAreaId('items'))){this.dataArea.onLoad=this.dataArea.onLoad.wrap(function(proceed){proceed();this._parent.itemsArea.setNode($(this._parent.getAreaId('items')));this._parent.itemsArea.onLoad();});this.itemsArea.onLoad=this.itemsArea.onLoad.wrap(function(proceed){proceed();if($(searchAreaId)&&!$(searchAreaId).visible()&&!$(searchButtonId)){this.addControlButton(searchButton);}});this.areasLoaded();this.itemsArea.onLoad();}}).bind(this));jQuery('#edit_form').on('submitQuote',function(){jQuery(this).trigger('realQuote');}).on('realQuote',this._realSubmit.bind(this));},areasLoaded:function(){},itemsLoaded:function(){jQuery('.action-add-tier').trigger('contentUpdated');jQuery('.action-delete-tier').trigger('contentUpdated');},dataLoaded:function(){this.dataShow();},setLoadBaseUrl:function(url){this.loadBaseUrl=url;},setAddresses:function(addresses){this.addresses=addresses;},addExcludedPaymentMethod:function(method){this.excludedPaymentMethods.push(method);},setCustomerId:function(id){this.customerId=id;this.loadArea('header',true);$(this.getAreaId('header')).callback='setCustomerAfter';$('back_quote_top_button').hide();$('reset_quote_top_button').show();},setCustomerAfter:function(){this.customerSelectorHide();if(this.storeId){$(this.getAreaId('data')).callback='dataLoaded';this.loadArea(['data'],true);}else{this.storeSelectorShow();}},setStoreId:function(id){this.storeId=id;this.storeSelectorHide();this.sidebarShow();this.dataShow();this.loadArea(['header','data'],true);},setCurrencyId:function(id){this.currencyId=id;this.itemsUpdate();},setCurrencySymbol:function(symbol){this.currencySymbol=symbol;},selectAddress:function(el,container){id=el.value;if(id.length==0){id='0';}
if(this.addresses[id]){this.fillAddressFields(container,this.addresses[id]);}else{this.fillAddressFields(container,{});}
var data=this.serializeData(container);data[el.name]=id;if(this.isShippingField(container)&&!this.isShippingMethodReseted){this.resetShippingMethod(data);}else{this.saveData(data);}},isShippingField:function(fieldId){if(this.shippingAsBilling){return fieldId.include('billing');}
...
var type=matchRes[1];var name=matchRes[2];var data;if(this.isBillingField(field.id)){data=this.serializeData(this.billingAddressContainer)}else{data=this.serializeData(this.shippingAddressContainer)}
data=data.toObject();if((type=='billing'&&this.shippingAsBilling)||(type=='shipping'&&!this.shippingAsBilling)){data['reset_shipping']=true;}
var customerAddressId=$('quote-'+type+'_address_customer_address_id').value;data['quote['+type+'_address][customer_address_id]']=customerAddressId;if(data['reset_shipping']){this.resetShippingMethod(data);}else{this.saveData(data);if(name=='country_id'||name=='customer_address_id'){this.loadArea(['shipping_method','billing_method','totals','convert','items'],true,data);}}},fillAddressFields:function(container,data){var regionIdElem=false;var regionIdElemValue=false;var fields=$(container).select('input','select','textarea');var re=/[^\[]*\[[^\]]*\]\[([^\]]*)\](\[(\d)\])?/;for(var i=0;i<fields.length;i++){if(fields[i].tagName.toLowerCase()=='input'&&fields[i].type.toLowerCase()=='file'){continue;}
...
if(name=='region'&&data['region_id']&&!data['region']){fields[i].value=data['region_id'];}}},disableShippingAddress:function(flag){this.shippingAsBilling=flag;if($('quote-shipping_address_customer_address_id')){$('quote-shipping_address_customer_address_id').disabled=flag;}
...
var buttons=$(this.shippingAddressContainer).select('button');for(var i=0;i<buttons.length;i++){buttons[i].disabled=flag;if(flag){buttons[i].addClassName('disabled');}else{buttons[i].removeClassName('disabled');}}}},setShippingAsBilling:function(flag){var data;var areasToLoad=['billing_method','shipping_address','totals','convert','giftmessage'];this.disableShippingAddress(flag);if(flag){data=this.serializeData(this.billingAddressContainer);}else{data=this.serializeData(this.shippingAddressContainer);areasToLoad.push('shipping_method');}
data=data.toObject();data['shipping_as_billing']=flag?1:0;data['reset_shipping']=1;this.loadArea(areasToLoad,true,data);},resetShippingMethod:function(data){var areasToLoad=['billing_method','shipping_address','totals','convert','giftmessage','items'];if(!this.isOnlyVirtualProduct){areasToLoad.push('shipping_method');areasToLoad.push('shipping_address');}
data['reset_shipping']=1;this.isShippingMethodReseted=true;this.loadArea(areasToLoad,true,data);},removeSelectedShippingMethod:function(){var areasToLoad=['shipping_method','totals'];this.loadArea(areasToLoad,true,{reset_shipping:1});},loadShippingRates:function(){this.isShippingMethodReseted=false;this.loadArea(['shipping_method','totals','convert'],true,{collect_shipping_rates:1});},setShippingMethod:function(method){var data={};data['quote[shipping_method]']=method;this.loadArea(['shipping_method','totals','convert','billing_method'],true,data);},setShippingMethodWithPrice:function(method,price){var data={};data['quote[shipping_method]']=method;data['quote[fixed_shipping_price]']=price;data['collect_shipping_rates']=1;this.loadArea(['shipping_method','totals','convert','billing_method'],true,data);},switchPaymentMethod:function(method){jQuery('#edit_form').trigger('changePaymentMethod',[method]);this.setPaymentMethod(method);var data={};data['quote[payment_method]']=method;this.loadArea(['card_validation'],true,data);},setPaymentMethod:function(method){if(this.paymentMethod&&$('payment_form_'+this.paymentMethod)){var form='payment_form_'+this.paymentMethod;[form+'_before',form,form+'_after'].each(function(el){var block=$(el);if(block){block.hide();block.select('input','select','textarea').each(function(field){field.disabled=true;});}});}
if(!this.paymentMethod||method){$('quote-billing_method_form').select('input','select','textarea').each(function(elem){if(elem.type!='radio'){elem.disabled=true;}})}
if($('payment_form_'+method)){jQuery('#'+this.getAreaId('billing_method')).trigger('contentUpdated');this.paymentMethod=method;var form='payment_form_'+method;[form+'_before',form,form+'_after'].each(function(el){var block=$(el);if(block){block.show();block.select('input','select','textarea').each(function(field){field.disabled=false;if(!el.include('_before')&&!el.include('_after')&&!field.bindChange){field.bindChange=true;field.paymentContainer=form;field.method=method;field.observe('change',this.changePaymentData.bind(this))}},this);}},this);}},changePaymentData:function(event){var elem=Event.element(event);if(elem&&elem.method){var data=this.getPaymentData(elem.method);if(data){this.loadArea(['card_validation'],true,data);}}},getPaymentData:function(currentMethod){if(typeof(currentMethod)=='undefined'){if(this.paymentMethod){currentMethod=this.paymentMethod;}else{return false;}}
...
return data;},applyCoupon:function(code){this.loadArea(['items','shipping_method','totals','convert','billing_method'],true,{'quote[coupon][code]':code,reset_shipping:false});this.quoteItemChanged=false;},applySubtotalProposal:function(amount,isPercentage){if(isPercentage){this.calculatePercentage(amount);}else{this.calculateSubtotalProposal(amount,isPercentage)}},calculatePercentage:function(discountPercentage){if(discountPercentage>0&&discountPercentage<=100){discountPercentage=100-discountPercentage;this.calculateSubtotalProposal(discountPercentage,true);}else{alert({title:jQuery.mage.__('Discount Percentage invalid'),content:jQuery.mage.__('Provide percentage of discount(for example: "50")'),})}},calculateSubtotalProposal:function(amount,isPercentage){this.loadArea(['sidebar','items','shipping_method','billing_method','totals','convert','giftmessage'],true,{'proposal[subtotal_proposal]':amount,'proposal[proposal_is_percentage]':isPercentage,reset_shipping:false});this.quoteItemChanged=true;},addProduct:function(id){this.loadArea(['items','shipping_method','totals','convert','billing_method'],true,{add_product:id,reset_shipping:true});},removeQuoteItem:function(id){this.loadArea(['items','shipping_method','totals','convert','billing_method'],true,{remove_item:id,from:'quote',reset_shipping:true});},moveQuoteItem:function(id,to){this.loadArea(['sidebar_'+to,'items','shipping_method','totals','convert','billing_method'],this.getAreaId('items'),{move_item:id,to:to,reset_shipping:true});},productGridShow:function(buttonElement){this.productGridShowButton=buttonElement;Element.hide(buttonElement);this.showArea('search');},productGridRowInit:function(grid,row){var checkbox=$(row).select('.checkbox')[0];var inputs=$(row).select('.input-text');if(checkbox&&inputs.length>0){checkbox.inputElements=inputs;for(var i=0;i<inputs.length;i++){var input=inputs[i];input.checkboxElement=checkbox;var product=this.gridProducts.get(checkbox.value);if(product){var defaultValue=product[input.name];if(defaultValue){if(input.name=='giftmessage'){input.checked=true;}else{input.value=defaultValue;}}}
...
this.loadArea(['items','shipping_method','billing_method','totals','convert','giftmessage'],true);}.bind(this));itemId=itemId?itemId:productId;productConfigure.showItemConfiguration(listType,itemId);return false;},removeSidebarItem:function(id,from){this.loadArea(['sidebar_'+from],'sidebar_data_'+from,{remove_item:id,from:from});},itemsUpdate:function(){var area=['sidebar','items','shipping_method','billing_method','totals','convert','giftmessage'];var fieldsPrepare={update_items:1};var info=$('quote-items_grid').select('input','select','textarea');for(var i=0;i<info.length;i++){if(!info[i].disabled&&(info[i].type!='checkbox'||info[i].checked)&&(info[i].type!='radio'||info[i].checked)){if(info[i].type=='select-one'){var select=info[i];var selectedOption=select.options[select.selectedIndex];if(selectedOption.dataset.action=='move-to-section'){fieldsPrepare[selectedOption.dataset.name]=selectedOption.value;}}
fieldsPrepare[info[i].name]=info[i].getValue();if(info[i].getValue()=='remove'){fieldsPrepare['remove_items']=1;}
if(info[i].getValue()=='duplicate'){fieldsPrepare['duplicate_items']=1;}}}
fieldsPrepare=Object.extend(fieldsPrepare,this.productConfigureAddFields);this.productConfigureSubmit('quote_items',area,fieldsPrepare);this.quoteItemChanged=false;},itemsOnchangeBind:function(){var elems=$('quote-items_grid').select('input','select','textarea');for(var i=0;i<elems.length;i++){if(!elems[i].bindOnchange){elems[i].bindOnchange=true;elems[i].observe('change',this.itemChange.bind(this))}}},itemChange:function(event){this.giftmessageOnItemChange(event);this.quoteItemChanged=true;},productConfigureSubmit:function(listType,area,fieldsPrepare,itemsFilter){area=this.prepareArea(area);this.loadingAreas=area;var url=this.loadBaseUrl+'block/'+area+'?isAjax=true';fieldsPrepare=this.prepareParams(fieldsPrepare);fieldsPrepare.reset_shipping=0;fieldsPrepare.json=1;var fields=[];for(var name in fieldsPrepare){fields.push(new Element('input',{type:'hidden',name:name,value:fieldsPrepare[name]}));}
productConfigure.addFields(fields);if(itemsFilter){productConfigure.addItemsFilter(listType,itemsFilter);}
productConfigure.addListType(listType,{urlSubmit:url});productConfigure.setOnLoadIFrameCallback(listType,function(response){this.loadAreaResponseHandler(response);}.bind(this));productConfigure.submit(listType);this.productConfigureAddFields={};},showQuoteItemConfiguration:function(itemId){var listType='quote_items';var qtyElement=$('quote-items_grid').select('input[name="item\['+itemId+'\]\[qty\]"]')[0];var selectedTier=$('quote-items_grid').select('input[name="item\['+itemId+'\]\[selected_tier\]"]')[0];productConfigure.setConfirmCallback(listType,function(){var confirmedCurrentQty=productConfigure.getCurrentConfirmedQtyElement();if(qtyElement&&confirmedCurrentQty&&!isNaN(confirmedCurrentQty.value)){qtyElement.value=confirmedCurrentQty.value;if(selectedTier&&selectedTier.value){var selectedTierQty=$('quote-items_grid').select('input[name="item\['+itemId+'\]\[tier_item\]\['+selectedTier.value+'\]\[qty\]"]')[0];if(selectedTierQty&&selectedTierQty.value){selectedTierQty.value=confirmedCurrentQty.value;}}}
this.productConfigureAddFields['item['+itemId+'][configured]']=1;}.bind(this));productConfigure.setShowWindowCallback(listType,function(){var formCurrentQty=productConfigure.getCurrentFormQtyElement();if(formCurrentQty&&qtyElement&&!isNaN(qtyElement.value)){formCurrentQty.value=qtyElement.value;}}.bind(this));productConfigure.showItemConfiguration(listType,itemId);},accountFieldsBind:function(container){if($(container)){var fields=$(container).select('input','select','textarea');for(var i=0;i<fields.length;i++){if(fields[i].id=='group_id'){fields[i].observe('change',this.accountGroupChange.bind(this))}else{fields[i].observe('change',this.accountFieldChange.bind(this))}}}},accountGroupChange:function(){this.loadArea(['data'],true,this.serializeData('quote-form_account').toObject());},accountFieldChange:function(){this.saveData(this.serializeData('quote-form_account'));},commentFieldsBind:function(container){if($(container)){var fields=$(container).select('input','textarea');for(var i=0;i<fields.length;i++){fields[i].observe('change',this.commentFieldChange.bind(this));}}},commentFieldChange:function(){this.saveData(this.serializeData('quote-comment'));},giftmessageFieldsBind:function(container){if($(container)){var fields=$(container).select('input','textarea');for(var i=0;i<fields.length;i++){fields[i].observe('change',this.giftmessageFieldChange.bind(this));}}},giftmessageFieldChange:function(){this.giftMessageDataChanged=true;},giftmessageOnItemChange:function(event){var element=Event.element(event);if(element.name.indexOf("giftmessage")!=-1&&element.type=="checkbox"&&!element.checked){var messages=$("quote-giftmessage").select('textarea');var name;for(var i=0;i<messages.length;i++){name=messages[i].id.split("_");if(name.length<2){continue;}
if(element.name.indexOf("["+name[1]+"]")!=-1&&messages[i].value!=""){alert({content:"First, clean the Message field in Gift Message form"});element.checked=true;}}}},loadArea:function(area,indicator,params){var deferred=new jQuery.Deferred();var url=this.loadBaseUrl;if(area){area=this.prepareArea(area);url+='block/'+area;}
if(indicator===true){indicator='html-body';}
params=this.prepareParams(params);params.json=true;if(!this.loadingAreas){this.loadingAreas=[];}
...
return params;},isPaymentValidationAvailable:function(){return((typeof this.paymentMethod)=='undefined'||this.excludedPaymentMethods.indexOf(this.paymentMethod)==-1);},serializeData:function(container){var fields=$(container).select('input','select','textarea');var data=Form.serializeElements(fields,true);return $H(data);},toggleCustomPrice:function(checkbox,elemId,tierBlock){if(checkbox.checked){$(elemId).disabled=false;$(elemId).show();if($(tierBlock)){$(tierBlock).hide();}}else{$(elemId).disabled=true;$(elemId).hide();if($(tierBlock)){$(tierBlock).show();}}},toggleProductRemark:function(checkbox,elemId,tierBlock){if(checkbox.checked){$(elemId).disabled=false;$(elemId).show();if($(tierBlock)){$(tierBlock).hide();}}else{$(elemId).disabled=true;$(elemId).hide();if($(tierBlock)){$(tierBlock).show();}}},submit:function(){jQuery('#edit_form').trigger('processStart');jQuery('#edit_form').trigger('submitQuote');},_realSubmit:function(){var disableAndSave=function(){disableElements('save');jQuery('#edit_form').on('invalid-form.validate',function(){enableElements('save');jQuery('#edit_form').trigger('processStop');jQuery('#edit_form').off('invalid-form.validate');});jQuery('#edit_form').triggerHandler('save');};if(this.quoteItemChanged){var self=this;jQuery('#edit_form').trigger('processStop');confirm({content:jQuery.mage.__('You have item changes. Save items changes?'),actions:{confirm:function(){self.itemsUpdate();jQuery('#edit_form').trigger('processStart');disableAndSave();},cancel:function(){jQuery("#edit_form").attr("action",jQuery("#edit_form").attr("data-default-action"));},}});}else{disableAndSave();}},edit:function(editActionUrl){var message='Are you sure you want to edit this quote? This will cancel this quote and create a new one.';confirm({content:jQuery.mage.__(message),actions:{confirm:function(){jQuery('#edit_form').attr('action',editActionUrl);jQuery('#edit_form').submit();}}});},cancel:function(cancelActionUrl){var message='Are you sure you want to cancel this quote? This will cancel the quote and notify the customer if the Cancel Quote template is enabled in the configuration.';confirm({content:jQuery.mage.__(message),actions:{confirm:function(){jQuery('#edit_form').attr('action',cancelActionUrl);jQuery('#edit_form').submit();}}});},duplicate:function(duplicateActionUrl){var message='Are you sure you want to duplicate this quote?';confirm({content:jQuery.mage.__(message),actions:{confirm:function(){jQuery('#edit_form').attr('action',duplicateActionUrl);jQuery('#edit_form').submit();}}});},overlay:function(elId,show,observe){if(typeof(show)=='undefined'){show=true;}
var quoteObj=this;var obj=this.overlayData.get(elId);if(!obj){obj={show:show,el:elId,quote:quoteObj,fx:function(event){this.quote.processOverlay(this.el,this.show);}};obj.bfx=obj.fx.bindAsEventListener(obj);this.overlayData.set(elId,obj);}else{obj.show=show;Event.stopObserving(window,'resize',obj.bfx);}
