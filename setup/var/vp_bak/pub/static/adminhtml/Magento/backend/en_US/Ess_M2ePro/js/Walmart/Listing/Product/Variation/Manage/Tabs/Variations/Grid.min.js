define(['M2ePro/Plugin/Messages','Magento_Ui/js/modal/modal','M2ePro/Listing/View/Grid','M2ePro/Walmart/Listing/View/Action','M2ePro/Walmart/Listing/Product/Template/Category','M2ePro/Walmart/Listing/Product/EditChannelData'],function(MessageObj,modal){window.WalmartListingProductVariationManageTabsVariationsGrid=Class.create(ListingViewGrid,{initialize:function($super,gridId,listingId){this.messageObj=Object.create(MessageObj);this.messageObj.setContainer('#listing_product_variation_action_messages_container');$super(gridId,listingId);},getLogViewUrl:function(rowId){var idField=M2ePro.php.constant('\\Ess\\M2ePro\\Block\\Adminhtml\\Log\\Listing\\Product\\AbstractGrid::LISTING_PRODUCT_ID_FIELD');var params={};params[idField]=rowId;return M2ePro.url.get('walmart_log_listing_product/index',params);},getComponent:function(){return'walmart';},getMaxProductsInPart:function(){return 1000;},prepareActions:function($super){this.actionHandler=new WalmartListingViewAction(this);this.actions={listAction:this.actionHandler.listAction.bind(this.actionHandler),relistAction:this.actionHandler.relistAction.bind(this.actionHandler),reviseAction:this.actionHandler.reviseAction.bind(this.actionHandler),stopAction:this.actionHandler.stopAction.bind(this.actionHandler),stopAndRemoveAction:this.actionHandler.stopAndRemoveAction.bind(this.actionHandler),previewItemsAction:this.actionHandler.previewItemsAction.bind(this.actionHandler),startTranslateAction:this.actionHandler.startTranslateAction.bind(this.actionHandler),stopTranslateAction:this.actionHandler.stopTranslateAction.bind(this.actionHandler)};this.templateCategoryHandler=new WalmartListingProductTemplateCategory(this);this.editChannelDataHandler=new WalmartListingProductEditChannelData(this);this.actions=Object.extend(this.actions,{deleteAndRemoveAction:this.actionHandler.deleteAndRemoveAction.bind(this.actionHandler)});},parseResponse:function(response){if(!response.responseText.isJSON()){return;}
...
return valid;}
var attr=variation.keys()[0];attributes[index].value=attr;var oldValue=options[index].value;options[index].update();options[index].enable();options[index].appendChild(new Element('option',{style:'display: none'}));if(type=='product'){var channelEl=$('new_child_product_channel_option_'+index);channelEl.update();channelEl.appendChild(new Element('option',{value:oldValue})).insert(oldValue);channelEl.value=oldValue;}
...
self.actionHandler.gridHandler.unselectAllAndReload();}});},createNewProductBtn:function(){var optEl=$('manage_variations_select_options'),newProductLinkEl=$('manage_variations_create_new_product');optEl&&optEl.show().down('input').enable();$('new_child_product_channel_options_error_row').setStyle({visibility:'hidden'});newProductLinkEl&&newProductLinkEl.hide();$$('.new-child-channel-option').each(function(el){el.disable();});$('manage_variations_create_new_product_title').show();ListingProductVariationManageVariationsGridObj.validateNewChildAttributeOptions('product');},selectOptionsBtn:function(){var optEl=$('manage_variations_select_options'),newProductEl=$('manage_variations_create_new_product');optEl&&optEl.hide().down('input').disable();$('new_child_product_channel_options_error_row').setStyle({visibility:'visible'});newProductEl&&newProductEl.show();$$('.new-child-channel-option').each(function(el){el.enable();el.update();});$('manage_variations_create_new_product_title').hide();ListingProductVariationManageVariationsGridObj.validateNewChildAttributeOptions('channel');},unselectAllAndReload:function($super){$super();ListingGridHandlerObj.variationProductManageHandler.reloadSettings(null);ListingGridHandlerObj.variationProductManageHandler.reloadVocabulary(null);}});});