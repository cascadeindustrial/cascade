...
EbayListingTemplateSwitcherObj.saveSwitchers(function(params){EbayListingViewSettingsGridObj.saveSettings(params);});}}],closed:function(){self.selectedProductsIds=[];self.selectedCategoriesData={};return true;}},'modal_setting_policy_action_dialog');this.insertHelpLink('modal_setting_policy_action_dialog');}.bind(this)});},openMotorsPopup:function(id){EbayListingViewSettingsMotorsObj.savedNotes={};this.selectedProductsIds=id?[id]:this.getSelectedProductsArray();EbayListingViewSettingsMotorsObj.openAddPopUp(this.selectedProductsIds);},saveSettings:function(savedTemplates){var requestParams={};$H(savedTemplates).each(function(i){requestParams[i.key]=i.value;});requestParams['ids']=this.selectedProductsIds.join(',');new Ajax.Request(M2ePro.url.get('ebay_template/saveListingProductsPolicy'),{method:'post',asynchronous:true,parameters:requestParams,onSuccess:function(transport){this.popUp.modal('closeModal');this.getGridObj().doFilter();}.bind(this)});},getSelectedProductsTitles:function(){if(this.selectedProductsIds.length>3){return'';}
...
return title;},transferring:function(id){this.selectedProductsIds=id?[id]:this.getSelectedProductsArray();if(this.selectedProductsIds.length){this.unselectAll();var productName=this.selectedProductsIds.length==1?this.getProductNameByRowId(this.selectedProductsIds[0]):null;EbayListingTransferringHandlerObj.loadActionHtml(this.selectedProductsIds,null,productName);}},confirm:function(config){if(config.actions&&config.actions.confirm){config.actions.confirm();}},insertHelpLink:function(popUpElementId){var modalHeader=jQuery('#'+popUpElementId).closest('.modal-inner-wrap').find('h1.modal-title');if(modalHeader.has('#popup_template_help_link')){modalHeader.find('#popup_template_help_link').remove();}
