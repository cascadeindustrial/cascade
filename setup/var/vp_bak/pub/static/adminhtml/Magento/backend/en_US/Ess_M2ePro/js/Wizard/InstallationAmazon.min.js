define(['M2ePro/Plugin/Messages'],function(MessagesObj){window.WizardInstallationAmazon=Class.create(Common,{continueStep:function(){if(WizardObj.steps.current.length){this[WizardObj.steps.current+'Step']();}},registrationStep:function(){WizardObj.registrationStep(M2ePro.url.get('wizard_installationAmazon/createLicense'));},marketplaceChange:function(){var marketplaceId=$('marketplace_id').value;if(!marketplaceId){return;}
...
$('amazon_wizard_installation_account_manual_authorization').show();$('manual_authorization_marketplace_application_name_container').show();$('manual_authorization_marketplace_developer_key_container_'+marketplaceId).show();$('manual_authorization_marketplace_register_url_container_'+marketplaceId).show();$('manual_authorization_marketplace_merchant_id_container_'+marketplaceId).show();$('manual_authorization_marketplace_token_container_'+marketplaceId).show();},accountStep:function(){if(!$('marketplace_id').value){this.alert(M2ePro.translator.translate('Please select Marketplace first.'));return;}
...
return setLocation(response['url']);}});},listingTutorialStep:function(){WizardObj.setStep(WizardObj.getNextStep(),setLocation.bind(window,location.href))}});});