...
var self=this;$('hide_item_help_'+rowId).observe('click',function(){self.hideItemHelp(rowId);});},createHelpTitleHtml:function(rowId){var closeHtml='<a href="javascript:void(0);" id="hide_item_help_'+rowId+'" title="'+M2ePro.translator.translate('Close')+'"><span class="hl_close icon-close"></span></a>';return'<div class="hl_header"><span class="hl_title">&nbsp;</span>'+closeHtml+'</div>';},createHelpViewAllLogHtml:function(rowId){var id=$('product_row_order_'+rowId).getAttribute('listing-product-pickup-store-state');return'<div class="hl_footer">'+'<a href="#" onclick="EbayListingPickupStoreGridObj.getLogGrid('+id+');">'+
M2ePro.translator.translate('View Full Product Log')+'</a></div>';},getLogGrid:function(rowId){new Ajax.Request(M2ePro.url.get('*/logGrid'),{method:'post',parameters:{listing_product_pickup_store_state:rowId},onSuccess:function(transport){var modalDialogMessage=$('pickup_store_variation_log_popup');if(!modalDialogMessage){modalDialogMessage=new Element('div',{id:'pickup_store_variation_log_popup'});this.logPopup=jQuery(modalDialogMessage).modal({title:M2ePro.translator.translate('Log For SKU '+rowId),type:'slide',buttons:[]});}
modalDialogMessage.up('.modal-inner-wrap').down('.modal-title').innerHTML=M2ePro.translator.translate('Log For SKU '+rowId);this.logPopup.modal('openModal');modalDialogMessage.innerHTML='';modalDialogMessage.insert(transport.responseText);modalDialogMessage.innerHTML.evalScripts();}});},updateButtonText:function(element,text){jQuery(element).find('span').text(M2ePro.translator.translate(text));},prepareActions:function(){var self=this;var actionHandler=new ListingViewAction(this);actionHandler.setProgressBar('pickup_store_view_progress_bar');actionHandler.setGridWrapper('pickup_store_view_content_container');actionHandler.setErrorsSummaryContainer('pickup_store_container_errors_summary');this.actions={unassignAction:function(){var selectedProductsParts=self.getSelectedItemsParts();if(selectedProductsParts.length==0){return;}
actionHandler.startActions(M2ePro.translator.translate('Unassign Product(s) from Stores'),M2ePro.url.get('*/unassign'),selectedProductsParts);}};},getMaxProductsInPart:function(){return 100;}});});