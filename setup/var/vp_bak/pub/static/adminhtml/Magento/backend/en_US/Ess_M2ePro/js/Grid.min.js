define(['M2ePro/Common'],function(){window.Grid=Class.create(Common,{initialize:function(gridId){this.gridId=gridId;this.prepareActions();},afterInitPage:function(){var submitButton=$$('#'+this.gridId+'_massaction-form button');submitButton.each((function(s){s.writeAttribute("onclick",'');s.observe('click',(function(){this.massActionSubmitClick();}).bind(this));}).bind(this));},getGridObj:function(){return window[this.gridId+'JsObject'];},getGridMassActionObj:function(){return window[this.gridId+'_massactionJsObject'];},getCellContent:function(rowId,cellIndex){var rows=this.getGridObj().rows;for(var i=0;i<rows.length;i++){var row=rows[i];var cells=$(row).childElements();var checkbox=$(cells[0]).select('input');checkbox=checkbox[0];if(checkbox&&checkbox.value==rowId){return trim(cells[cellIndex].innerHTML);}}
return'';},getProductNameByRowId:function(rowId){var cellContent=this.getCellContent(rowId,this.productTitleCellIndex);var expr=new RegExp(/<span[^>]*>(.*?)<\/span>/i);var matches=expr.exec(cellContent);return(matches&&!Object.isUndefined(matches[1]))?matches[1]:'';},selectAll:function(){this.getGridMassActionObj().selectAll();},unselectAll:function(){this.getGridMassActionObj().unselectAll();},unselectAllAndReload:function(){this.unselectAll();this.getGridObj().reload();},selectByRowId:function(rowId){this.unselectAll();var rows=this.getGridObj().rows;for(var i=0;i<rows.length;i++){var row=rows[i];var cells=$(row).childElements();var checkbox=$(cells[0]).select('input');checkbox=checkbox[0];if(checkbox.value==rowId){checkbox.checked=true;this.getGridMassActionObj().checkedString=rowId.toString();break;}}},getSelectedProductsString:function(){return this.getGridMassActionObj().checkedString},getSelectedProductsArray:function(){return this.getSelectedProductsString().split(',');},getOrderedSelectedProductsArray:function(){var selectedProductsArray=this.getSelectedProductsArray();var checkboxesValuesArray=this.getGridMassActionObj().getCheckboxesValuesAsString().split(',');var orderedSelectedProductArray=[];checkboxesValuesArray.forEach(function(value){if(selectedProductsArray.indexOf(value)>=0){orderedSelectedProductArray.push(value);}});return orderedSelectedProductArray;},massActionSubmitClick:function(){var self=this;if(self.getSelectedProductsString()==''||self.getSelectedProductsArray().length==0){self.alert(M2ePro.translator.translate('Please select Items.'));return;}
...
self.actions[o.value+'Action']();});},cancel:function(){return false;}}});},viewItemHelp:function(rowId,data,hideViewLog){$('grid_help_icon_open_'+rowId).hide();$('grid_help_icon_close_'+rowId).show();if($('grid_help_content_'+rowId)!=null){$('grid_help_content_'+rowId).show();return;}
...
var self=this;$('hide_item_help_'+rowId).observe('click',function(){self.hideItemHelp(rowId);});},hideItemHelp:function(rowId){if($('grid_help_content_'+rowId)!=null){$('grid_help_content_'+rowId).hide().remove();}
$('grid_help_icon_open_'+rowId).show();$('grid_help_icon_close_'+rowId).hide();},createHelpTitleHtml:function(rowId){var productTitle=this.getProductNameByRowId(rowId);var closeHtml='<a href="javascript:void(0);" id="hide_item_help_'+rowId+'" title="'+M2ePro.translator.translate('Close')+'"><span class="hl_close icon-close"></span></a>';return'<div class="hl_header"><span class="hl_title">'+productTitle+'</span>'+closeHtml+'</div>';},createSynchNoteHtml:function(synchNote){return'<div class="messages m2epro-messages a-left">'+'<div class="message message-warning error">'+synchNote+'</div></div></div>';},createHelpActionHtml:function(action){var self=this;var classContainer='hl_container';if(action.type==M2ePro.php.constant('Ess_M2ePro_Model_Log_AbstractModel::TYPE_SUCCESS')){classContainer+=' hl_container_success';}else if(action.type==M2ePro.php.constant('Ess_M2ePro_Model_Log_AbstractModel::TYPE_WARNING')){classContainer+=' hl_container_warning';}else if(action.type==M2ePro.php.constant('Ess_M2ePro_Model_Log_AbstractModel::TYPE_ERROR')){classContainer+=' hl_container_error';}else{classContainer+=' hl_container_notice';}
var html='<div class="'+classContainer+'">';html+='<div class="hl_date">'+action.localized_date+'</div>'+'<div class="hl_action">';if(action.initiator!=''){html+='<strong style="color: gray;">'+action.initiator+'</strong>&nbsp;&nbsp;';}
...
html+='</div>'+'</div>';return html;},createHelpViewAllLogHtml:function(rowId){return'<div class="hl_footer"><a target="_blank" href="'+this.getLogViewUrl(rowId)+'">'+
M2ePro.translator.translate('View Full Product Log')+'</a></div>';},getLogViewUrl:function(rowId){alert('abstract getLogViewUrl');},prepareActions:function(){alert('abstract prepareActions');},getComponent:function(){alert('abstract getComponent');}});});