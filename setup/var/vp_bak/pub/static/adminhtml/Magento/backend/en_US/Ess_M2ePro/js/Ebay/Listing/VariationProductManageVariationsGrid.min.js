define(['jquery','M2ePro/Grid'],function(jQuery){window.EbayListingVariationProductManageVariationsGrid=Class.create(Grid,{initialize:function($super,gridId){$super(gridId);jQuery.validator.addMethod('M2ePro-upc',function(value,el){if(value.length==0){return true;}
...
return check%10==0;}
return false;},M2ePro.translator.translate('Please enter valid ISBN'));this.initFormValidation('form[id^="variation_identifiers_edit_"]');},getComponent:function(){return'ebay';},getMaxProductsInPart:function(){return 1000;},prepareActions:function(){return false;},afterInitPage:function($super){$super();$$('.attributes-options-filter').each(this.initAttributesOptionsFilter,this);},parseResponse:function(response){if(!response.responseText.isJSON()){return;}
return response.responseText.evalJSON();},initAttributesOptionsFilter:function(filterEl){var srcElement=Element.down(filterEl,'select');srcElement.observe('change',this.onAttributesOptionsFilterChange.bind(this));var valuesDiv=Element.down(filterEl,'.attributes-options-filter-values');valuesDiv.optionsCount=valuesDiv.childElementCount;if(valuesDiv.optionsCount==srcElement.childElementCount-1){srcElement.hide();}
