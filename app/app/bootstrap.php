...
//     $logger->info(print_r($myArray, true)); // Array Log
//     if ($exit) {
//         exit;
//     }
    function printCustomLog($myArray, $exit=0)
{
    $writer = new \Zend\Log\Writer\Stream(BP . '/var/log/splittax.log');
    $logger = new \Zend\Log\Logger();
    $logger->addWriter($writer);
    $logger->info(print_r($myArray, true)); // Array Log
    if ($exit) {
...
function printLog($myArray, $exit=0)
{
    $writer = new \Zend\Log\Writer\Stream(BP . '/var/log/splitcart.log');
    $logger = new \Zend\Log\Logger();
    $logger->addWriter($writer);
    $logger->info(print_r($myArray, true)); // Array Log
    if ($exit) {
...
function printShipperLog($myArray, $exit=0)
{
    $writer = new \Zend\Log\Writer\Stream(BP . '/var/log/shqdebug.log');
    $logger = new \Zend\Log\Logger();
    $logger->addWriter($writer);
    $logger->info(print_r($myArray, true)); // Array Log
    if ($exit) {
