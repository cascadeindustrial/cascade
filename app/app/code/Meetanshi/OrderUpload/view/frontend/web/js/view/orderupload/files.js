            ...
            init: function (element, valueAccessor) {
                if (quoteData.file_data) {
                    ...
                    for (var i=0; i<fileObj.length; i++){
                        ...
                        jQuery('div.filePreview').append('<p id="'+cls+'" class="orderupload-link"><a target="_blank" href="' + fileUrl + '">' + fileObj[i]['name'] + '</a><span class="remove-btn inits"><a class="'+cls+'" href="javascript:void(0)" data-value="'+fileObj[i]['name']+'">+remove+</a></span></p>');
                    }
                }
                ...
                $('#fileupload').fileupload({
                    add: function (e, data) {
                        ...
                        if ($.inArray(ext, extArray) === -1) {
                            uploadError.push('The file you attached is not valid, try again with a valid file type attachment.');
                        }
                        if (data.originalFiles[0].size > (maxFileSize * 1024 * 1024)) {//2 MB
                            uploadError.push('Filesize is too big');
                        }
                    },
                    start: function (e) {
                         alert("from start");
                    ...
                    $.ajax({
                        ...
                        success: function (data) {
                            ...
                            for (var i = 0; i < data.length; i++) {
                                ...
                                jQuery('div.filePreview').append('<p id="'+cls+'" class="orderupload-link"><a href="' + fileUrl + '">' + data[i].name + '</a><span class="remove-btn uploaded"><a class="'+cls+'" href="javascript:void(0)" data-value="'+data[i].name+'">Remove</a></span></p>');
                            }
