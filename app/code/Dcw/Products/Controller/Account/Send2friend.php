...
namespace Dcw\Products\Controller\Account;
 ...
 */
class Send2friend extends \Cminds\Creditline\Controller\Account\Send2friend
{
    /**
     *
     * @return void
        ...
        if ($email && $amount > 0) {
            ...
            if (!$error) {
                ...
                if ($myBalance->getAmount() >= $baseAmount) {
                    ...
                    $this->messageManager->addSuccessMessage(__('Credit Shared Successfully.'));

                    $this->customerSession->setSend2FriendFormData(false);
                } else {
                    $this->customerSession->setSend2FriendFormData($this->getRequest()->getParams());

                    $this->messageManager->addErrorMessage(__('Insufficient credit available.'));
                }
            } else {
                $this->customerSession->setSend2FriendFormData($this->getRequest()->getParams());
