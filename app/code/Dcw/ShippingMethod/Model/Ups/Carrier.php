...
namespace Dcw\ShippingMethod\Model\Ups;
...
class Carrier extends AbstractCarrierOnline implements CarrierInterface
{
    ...
    if(false)
    {
     ...
     $weight = $this->setRequest($request)->getWeight();*/
     //$finalWeight = $weight;
    ...
    public function setRequest(RateRequest $request)
    {
        $this->_request = $request;
          $logFile='/var/log/ups_request.log';
         $writer = new \Zend\Log\Writer\Stream(BP . $logFile);
         $logger = new \Zend\Log\Logger();
         $logger->addWriter($writer);
         $logger->info(json_encode($this->_request->getData()));
        ...
        $logger->info('getOrigPostcode');
 $logger->info($request->getOrigPostcode());
 ...
 $logger->info($request->getOrigPostcode());
    ...
    public function getShipmentByCode($code, $origin = null)
    {
        ...
        $logFile='/var/log/ups_saturday.log';
        $writer = new \Zend\Log\Writer\Stream(BP . $logFile);
        $logger = new \Zend\Log\Logger();
        $logger->addWriter($writer);
        //$logger->info($arr);
     ...
     */
     protected function _getXmlQuotes()
     {
         ...
         $rowRequest = $this->_rawRequest;
          ...
          $logger->info("rowRequest");
          $logger->info($rowRequest->getData());
         if (self::USA_COUNTRY_ID == $rowRequest->getDestCountry()) {
             $destPostal = substr((string)$rowRequest->getDestPostal(), 0, 5);
         } else {
             $destPostal = $rowRequest->getDestPostal();
...
XMLRequest;
         ...
         $satDelivery = $this->_scopeConfig->getValue('carriers/ups/saturday_delivery', $storeScope);
         ...
         //
         // $logger->info("XML Request1");
         // $logger->info($xmlRequest1);
         ...
         if($satDelivery==1 && ($day=="Thu" || $day=="Fri"))
         {
            $logger->info("in if loop condition");

           $xmlParams1 = <<<XMLRequest
...
XMLRequest;
       ...
       $xmlRequest1 = $xmlRequest.$xmlParams.$xmlParams1;
...
XMLRequest;
       ...
       $httpResponse1 = $this->asyncHttpClient->request(
           new Request($url, Request::METHOD_POST, ['Content-Type' => 'application/xml'], $xmlRequest1)
       );
       $xmlResponse1 = $httpResponse1->get()->getBody();

        $logger->info("XML Response");
          $logger->info($xmlResponse1);

       return $this->deferredProxyFactory->create(
           [
               'deferred' => new CallbackDeferred(
                   function () use ($httpResponse,$httpResponse1) {
                       ...
                       return $this->_parseXmlResponse($xmlResponse,$xmlResponse1);
                   }
               )
           ]
       );
           ...
           $logger->info("in if else loop condition");
...
XMLRequest;
     ...
     */
    protected function _parseXmlResponse($xmlResponse,$xmlResponse1='')
    {
        $logFile='/var/log/ups_saturday.log';
        $writer = new \Zend\Log\Writer\Stream(BP . $logFile);
        $logger = new \Zend\Log\Logger();
        ...
        $storeScope = \Magento\Store\Model\ScopeInterface::SCOPE_STORE;
        $satDelivery = $this->_scopeConfig->getValue('carriers/ups/saturday_delivery', $storeScope);
        /*** For Saturday shipping [start here] **/
        if ($satDelivery == 1 && $xmlResponse1!='') {
            if (strlen(trim($xmlResponse1)) > 0) {
            $xml = new \Magento\Framework\Simplexml\Config();
            $xml->loadString($xmlResponse1);
            $arr1 = $xml->getXpath("//RatingServiceSelectionResponse/Response/ResponseStatusCode/text()");
            $success = (int)$arr1[0];
            if ($success === 1) {
                $arr1 = $xml->getXpath("//RatingServiceSelectionResponse/RatedShipment");
                $allowedMethods = explode(",", $this->getConfigData('allowed_methods'));

                // Negotiated rates
                $negotiatedArr1 = $xml->getXpath("//RatingServiceSelectionResponse/RatedShipment/NegotiatedRates");
                $negotiatedActive1 = $this->getConfigFlag('negotiated_active')
                    && $this->getConfigData('shipper_number')
                    && !empty($negotiatedArr1);

                $allowedCurrencies = $this->_currencyFactory->create()->getConfigAllowCurrencies();
                // $logger->info("Arr value:");
                // $logger->info($arr);
                foreach ($arr1 as $shipElement) {
                    $this->processShippingRateForItem1(
                        $shipElement,
                        $allowedMethods,
                        $allowedCurrencies,
                        $costArr1,
                ...
                $arr1 = $xml->getXpath("//RatingServiceSelectionResponse/Response/Error/ErrorDescription/text()");
                $errorTitle = (string)$arr1[0][0];
                $error = $this->_rateErrorFactory->create();
                $error->setCarrier('ups');
                $error->setCarrierTitle($this->getConfigData('title'));
                $error->setErrorMessage($this->getConfigData('specificerrmsg'));
           ...
           $logger->info("Final xml output for ups");
        $logger->info($priceArr);
        }
    }
        /*** For Saturday shipping [ends here] **/

        $logger->info("Final xml output for ups");
        $logger->info($priceArr);

        $logger->info("Final xml output for ups1111111");
        $logger->info($priceArr1);
        ...
        if (empty($priceArr)) {
            ...
            foreach ($priceArr as $method => $price) {
                ...
                $guaranteedDeliveryDays = $price['days'];
                $scheduledTime = $price['scheduledtime'];
                if($guaranteedDeliveryDays)
                {
                    $finalDateTime = date('Y-m-d', strtotime($currentDateTime. ' + '.$guaranteedDeliveryDays.' days'));
                    $time=strtotime($finalDateTime);
                    $day=date("d",$time);
                    $month=date("F",$time);
                    $year=date("Y",$time);
                    ...
                    if($scheduledTime)
                    {
                      $formattedDeliveryTime = str_replace("Noon","P.M",$scheduledTime);
                      $deliveryMessage = "Estimated delivery by $modifiedDay $month"." ".$year." ".$formattedDeliveryTime;
                    }
                    else
                      $deliveryMessage = "Estimated delivery by $modifiedDay $month"." ".$year;
                }
                else{
                  $deliveryMessage = '';
                }
                ...
                // $logger->info("Method Title");
                // $logger->info($methodArr);
                $rate->setCost($costArr[$method]);
                $rate->setPrice($price['price']);
                $result->append($rate);
            }
            foreach ($priceArr1 as $method => $price) {
                //$logger->info("in second foreach loop");
                $rate = $this->_rateMethodFactory->create();
                $rate->setCarrier('ups');
                $rate->setCarrierTitle($this->getConfigData('title'));
                $rate->setMethod($method."sat");
                $methodArr = $this->getShipmentByCode($method);
                $rate->setMethodTitle("Saturday ".$methodArr);
                $guaranteedDeliveryDays = $price['days'];
                $scheduledTime = $price['scheduledtime'];
                if($guaranteedDeliveryDays)
                {
                    ...
                    $year=date("Y",$time);
                    if($scheduledTime)
                      $deliveryMessage = "Estimated delivery by $day"."th $month"." ".$year." ".$scheduledTime;
                    else
                      $deliveryMessage = "Estimated delivery by $day"."th $month"." ".$year;
                }
                else{
                  $deliveryMessage = '';
                }
                $rate->setDescription($deliveryMessage);
                //$rate->setDescription($price['days']);
                // $logger->info("Method Title");
                // $logger->info("Saturday".$methodArr);
                $rate->setCost($costArr1[$method]);
                $rate->setPrice($price['price']);
                $result->append($rate);
            }
        }
    ...
    private function processShippingRateForItem(
        ...
        $guaranteedDelivery = $shipElement->GuaranteedDaysToDelivery;
        $scheduledTime = $shipElement->ScheduledDeliveryTime;
        $logFile='/var/log/ups_saturday.log';
        $writer = new \Zend\Log\Writer\Stream(BP . $logFile);
        $logger = new \Zend\Log\Logger();
        $logger->addWriter($writer);
        $guaranteedDelivery = (string)$shipElement->GuaranteedDaysToDelivery;
        $scheduledTime = (string)$shipElement->ScheduledDeliveryTime;
        $logger->info("code");
        $logger->info($code);
        $logger->info("guaranteeDelivery");
        $logger->info($guaranteedDelivery);
        ...
        if (in_array($code, $allowedMethods)) {
            ...
            if ($successConversion) {
                $costArr[$code] = $cost;
                $methodPrice = $this->getMethodPrice((float)$cost, $code);
                $priceArr[$code] = array('price'=>$methodPrice,'days'=>$guaranteedDelivery,"scheduledtime"=>$scheduledTime);

            }
        }
    }

    /** For Saturday shipping [starts here] **/
     ...
     * @SuppressWarnings(PHPMD.CyclomaticComplexity)
     */
    private function processShippingRateForItem1(
        \Magento\Framework\Simplexml\Element $shipElement,
        array $allowedMethods,
        array $allowedCurrencies,
        array &$costArr1,
        ...
        // if($day=="Thu" && $guaranteedDelivery==1)
        // {
        //
        // }

        if($day=="Thu" && $guaranteedDelivery==2)
        {
          if (in_array($code, $allowedMethods)) {
              //The location of tax information is in a different place
              // depending on whether we are using negotiated rates or not
              ...
              if ($successConversion) {
                  $costArr1[$code] = $cost;
                  $methodPrice = $this->getMethodPrice((float)$cost, $code);
                  $priceArr1[$code] = array('price'=>$methodPrice,'days'=>$guaranteedDelivery,"scheduledtime"=>$scheduledTime);
                  //$deliveryTime[$code] = array('days'=>$guaranteedDelivery,"scheduledtime"=>$scheduledTime);
              }
          }
        }
        if($day=="Fri" && $guaranteedDelivery==1)
        {
          if (in_array($code, $allowedMethods)) {
              //The location of tax information is in a different place
              // depending on whether we are using negotiated rates or not
