<?php
namespace Cart2Quote\AmastyCompatability\Plugin\Model\Quote\CheckoutInitializationPlugin;

/**
 * Interceptor class for @see \Cart2Quote\AmastyCompatability\Plugin\Model\Quote\CheckoutInitializationPlugin
 */
class Interceptor extends \Cart2Quote\AmastyCompatability\Plugin\Model\Quote\CheckoutInitializationPlugin implements \Magento\Framework\Interception\InterceptorInterface
{
    use \Magento\Framework\Interception\Interceptor;

    public function __construct(\Amasty\Checkout\Model\Config $checkoutConfig, \Amasty\Checkout\Model\FieldsDefaultProvider $defaultProvider, \Magento\Framework\Webapi\ServiceOutputProcessor $outputProcessor, \Magento\Quote\Api\CartRepositoryInterface $quoteRepository, \Amasty\Checkout\Model\Quote\Shipping\AddressMethods $addressMethods, \Magento\Payment\Model\MethodList $paymentMethodList, \Magento\Quote\Api\CartTotalRepositoryInterface $cartTotalsRepository, \Cart2Quote\Quotation\Model\Session $quotationSession)
    {
        $this->___init();
        parent::__construct($checkoutConfig, $defaultProvider, $outputProcessor, $quoteRepository, $addressMethods, $paymentMethodList, $cartTotalsRepository, $quotationSession);
    }

    /**
     * {@inheritdoc}
     */
    public function beforeInitializeShipping(\Amasty\Checkout\Model\Quote\CheckoutInitialization $subject, \Magento\Quote\Api\Data\CartInterface $quote)
    {
        $pluginInfo = $this->pluginList->getNext($this->subjectType, 'beforeInitializeShipping');
        if (!$pluginInfo) {
            return parent::beforeInitializeShipping($subject, $quote);
        } else {
            return $this->___callPlugins('beforeInitializeShipping', func_get_args(), $pluginInfo);
        }
    }
