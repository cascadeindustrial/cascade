<?php
namespace Cart2Quote\Quotation\Plugin\Magento\Reports\Model\ResourceModel\Customer\CollectionPlugin;

/**
 * Interceptor class for @see \Cart2Quote\Quotation\Plugin\Magento\Reports\Model\ResourceModel\Customer\CollectionPlugin
 */
class Interceptor extends \Cart2Quote\Quotation\Plugin\Magento\Reports\Model\ResourceModel\Customer\CollectionPlugin implements \Magento\Framework\Interception\InterceptorInterface
{
    use \Magento\Framework\Interception\Interceptor;

    public function __construct(\Magento\Framework\Data\Collection\EntityFactory $entityFactory, \Psr\Log\LoggerInterface $logger, \Magento\Framework\Data\Collection\Db\FetchStrategyInterface $fetchStrategy, \Magento\Framework\Event\ManagerInterface $eventManager, \Magento\Eav\Model\Config $eavConfig, \Magento\Framework\App\ResourceConnection $resource, \Magento\Eav\Model\EntityFactory $eavEntityFactory, \Magento\Eav\Model\ResourceModel\Helper $resourceHelper, \Magento\Framework\Validator\UniversalFactory $universalFactory, \Magento\Framework\Model\ResourceModel\Db\VersionControl\Snapshot $entitySnapshot, \Magento\Framework\DataObject\Copy\Config $fieldsetConfig, \Magento\Quote\Api\CartRepositoryInterface $quoteRepository, \Magento\Quote\Model\ResourceModel\Quote\Item\CollectionFactory $quoteItemFactory, \Magento\Sales\Model\ResourceModel\Order\Collection $orderResource, \Cart2Quote\Quotation\Model\ResourceModel\Quote\Collection $quoteResource, ?\Magento\Framework\DB\Adapter\AdapterInterface $connection = null, $modelName = 'Magento\\Customer\\Model\\Customer')
    {
        $this->___init();
        parent::__construct($entityFactory, $logger, $fetchStrategy, $eventManager, $eavConfig, $resource, $eavEntityFactory, $resourceHelper, $universalFactory, $entitySnapshot, $fieldsetConfig, $quoteRepository, $quoteItemFactory, $orderResource, $quoteResource, $connection, $modelName);
    }

    /**
     * {@inheritdoc}
     */
    public function aroundLoad($subject, $proceed, $printQuery = false, $logQuery = false)
    {
        $pluginInfo = $this->pluginList->getNext($this->subjectType, 'aroundLoad');
        if (!$pluginInfo) {
            return parent::aroundLoad($subject, $proceed, $printQuery, $logQuery);
        } else {
            return $this->___callPlugins('aroundLoad', func_get_args(), $pluginInfo);
        }
    }
